...
│   ├── events/
│   └── internal/
├── apigov/         # API Access Governance: govern outbound REST/SOAP/GraphQL/gRPC calls — connectors, schema ingestion, permissions, submit→AI→review→execute pipeline, masking, break-glass, text-to-API (AF-500); connector-level response masking policies + data-classification tags with auto-derived masking & AI-risk bump (schema-field/JSON-path/XML-path/regex matchers — AF-518); read-only decision trace over a pure `ApiDecisionEvaluator` — `POST /admin/api-call-simulations` replays one hypothetical call through the nine stages the live pipeline runs, never contacting the governed API (#967)
│   ├── api/
│   ├── events/
│   └── internal/   # persistence, client (per-protocol exec + auth + prober), schema (parsers), routing, scheduled, web
├── deploygov/      # Deployment approval governance (epic AF-682): gate CI/CD deployments behind approval workflows. #684 landed the persistence foundation — pipelines/environments/freeze windows/requests/decisions/routing/grants (reusing query_status, submission_reason, decision, api_routing_action) + DEPLOYMENT_PIPELINE_MANAGE / DEPLOYMENT_REVIEW permissions; #688 adds the admin surface — pipeline/environment/permission/freeze-window CRUD services + controllers, the effective-permission resolver (most-permissive union of direct + unexpired group grants), and the fail-closed FreezeWindowEvaluator; #691 adds the submission half — the API-key-authenticated trigger endpoint (idempotent on the CI run), `ai.api.DeploymentAnalyzer` + the deploygov-side analysis listener, the typed-conditions DeploymentRoutingPolicyEngine with admin CRUD, and the transition-validating DeploymentRequestStateService; #692 adds the human half — the review decision endpoints (`/api/v1/deployment-reviews`, single-stage, idempotent, self-approval → 409, plan-approver eligibility without delegation), deployment break-glass (can_break_glass grant AND environment allow_break_glass, no admin bypass; force-approve, freeze bypass audited via DEPLOYMENT_BREAK_GLASS_EXECUTED, mandatory retro-review in break_glass_events via the synchronous workflow listener), and the plan-driven DeploymentTimeoutJob (`accessflow.deploygov.timeout-check`); #693 adds the machine contract — the fail-closed gate (`GET /api/v1/deployment-gate`, one pure releasable function: APPROVED, no active freeze window (HOLD or REJECT; break-glass exempt), scheduled_for passed; any error → not releasable; 404-never-403 visibility), confirm-execution (`APPROVED → EXECUTED`, releasable-gated, audited DEPLOYMENT_EXECUTED trigger=pipeline), idempotent outcome reporting (conflict → 409 DEPLOYMENT_OUTCOME_CONFLICT; FAILED flips EXECUTED → FAILED; ROLLED_BACK on a require_review environment opens a deployment_rollback_reviews follow-up the submitter can never acknowledge), and the notify-only ScheduledDeploymentReleaseJob (`accessflow.deploygov.release-check`, one-shot DeploymentReleasableEvent via release_notified_at); #694 adds the CI wrappers — the `.github/actions/deployment-gate` + `deployment-outcome` composite actions, the `.accessflow_deployment_gate`/`_outcome` GitLab hidden jobs (`ci-templates/gitlab/accessflow-deployment.gitlab-ci.yml`), the Azure step template (`ci-templates/azure/`), and the generic curl walkthrough — all submit-idempotently-by-run-id → poll-gate → confirm → report-outcome, fail-closed (gate 404 or any non-retryable error fails the job), pipeline identified by UUID (a trigger-only key cannot resolve names), offline-tested via the fake-curl harness in `.github/actions/tests/`; #695 adds the human fan-out — five `DEPLOYMENT_*` NotificationEventType values (submitted-on-PENDING_REVIEW → eligible reviewers, approved/rejected → submitter, outcome-failed → the granting approvers, break-glass → all admins + PagerDuty via the existing BREAK_GLASS trigger), `notifications/internal/DeploymentNotificationListener` + `deploygov/api/DeploymentNotificationLookupService`, `user_notifications.deployment_request_id` (V155), audit actions for every remaining transition (SUBMITTED/APPROVED/REJECTED/TIMED_OUT/CANCELLED + DEPLOYMENT_BREAK_GLASS_REVIEWED; system rows carry null actor + `trigger=routing|freeze|timeout|environment_policy`), and no ticketing (queries never ticket on submission either; admin-CRUD audit moved to a follow-up); #696 adds the web UI — `/deployments`, `/deployments/:id`, `/deployment-reviews` (with the rollback worklist; since #772 both are tabs of the unified `/reviews` hub — `?tab=deployments|rollbacks` — and the old URL redirects), `/admin/deployment-pipelines` and its settings tabs (environments, permissions, freeze windows, routing policies, CI snippet panel), all `t()`-keyed under `deploygov.*`; #697 is the docs/website sweep and adds [`docs/18-deployment-governance.md`](docs/18-deployment-governance.md) — read that chapter before touching the module; #741 adds multi-environment version tracking — free-form `tags` on deployment_environments (≤10, ≤32 chars, the query_templates.tags precedent) and the one-row-per-environment `deployment_environment_versions` read model maintained by the module-private DeploymentVersionTrackerService inside the EXECUTED/outcome transactions (shift-on-execute, single-level revert on FAILED/ROLLED_BACK for the current request, non-current outcomes no-op; never feeds gate/approval/routing; UI follows in #743); #742 adds the read-only version inventory & drift API — `deploygov/api/DeploymentVersionInventoryService` behind three endpoints (per-pipeline matrix, org-wide filterable matrix, per-environment history straight from deployment_requests), drift computed at read time only (no job, no notifications, no semver — plain string inequality vs the pipeline's newest successful deploy over the UNFILTERED row set), timed by `deployment_requests.executed_at` (V157, stamped at the state-service EXECUTED chokepoint); per-pipeline visibility = MANAGE/REVIEW/QUERY_ADMIN/effective can_trigger failing 404-never-403, org-wide matrix functional-permission-only; #967 adds the decision trace — a pure `DeploymentDecisionEvaluator` split out of the review state machine, and `POST /admin/deployment-simulations`, which walks the trigger grant, freeze window, routing, environment policy, approvers, schedule and the gate's own `releasable` function at an optional hypothetical instant `at`; #877 (epic #870, schema change sets) makes environments schema-promotion targets — nullable `deployment_environments.datasource_id` (bare UUID, no FK, validated via `core.api.DatasourceAdminService.getForAdmin` → the existing `404 DATASOURCE_NOT_FOUND`; `clearDatasource` unbinds) and a **real ladder**: V177 backfills `sort_order` to distinct contiguous values per pipeline and adds `UNIQUE (pipeline_id, sort_order)`; the service appends an omitted position at `max+1`, pre-checks an explicit duplicate and translates the raced unique violation to `409 DEPLOYMENT_ENVIRONMENT_SORT_ORDER_CONFLICT` (never default `sort_order` to `0` again — a "lower-ordered environments applied" gate over all-zero rows evaluates the empty set and fails open); two narrow api lookups — `DeploymentEnvironmentLookupService` (ladder + one-by-id, not org-scoped: resolve the pipeline first) and `DeploymentFreezeLookupService` (pure delegation to the package-private `FreezeWindowEvaluator`, preserving fail-closed `HOLD` / most-specific / `REJECT`-beats-`HOLD`) — exist so `schemachange` never imports `deploygov.internal`
│   ├── api/
│   ├── events/
...
├── sqlreview/      # Deterministic SQL review rules with per-environment severity (epic #860). #861 lands the persistence foundation only — PG enums datasource_environment + sql_review_severity (OFF/WARN/BLOCK), the nullable datasources.environment column, sql_review_rulesets (one per environment per org + at most one org default, two partial unique indexes) / sql_review_rule_configs / query_sql_review_findings, the api contracts (SqlReviewService, SqlReviewRulesetService — unimplemented until #862/#863) and the SQL_REVIEW_MANAGE permission (WORKFLOW_ADMIN, ADMIN-only, seeded by V171). Cross-module refs are bare UUIDs. BLOCK never rejects — it only suppresses auto-approval (#864). #862 adds the engine — `internal/rules/SqlRule` (internal, not api: rules import JSqlParser) with fourteen AST-only built-ins in `SqlRuleCatalog`, the pure `SqlReviewEvaluator` (OFF skipped, a throwing rule skipped per statement, findings ordered statement → line → rule id), `DefaultSqlReviewService` (explicit relational allow-list → `applicable:false` for every plugin DbType before parsing; datasource environment ruleset → org default → none; a disabled bound ruleset = no rules, no fall-through; unconfigured rules run at their built-in default severity), `SqlRuleParamsCodec` (`params` JSONB = object of string arrays) and the write-time `SqlRuleParamsValidator` for #863; 42 `sqlreview.rule.<id>.{name,description,message}` keys in every messages file; envelope members carry null line numbers. #863 adds the REST surface — `/admin/sql-review-rulesets` CRUD (`SQL_REVIEW_MANAGE`; pre-checked 409 `SQL_REVIEW_RULESET_ENVIRONMENT_CONFLICT` / `_DEFAULT_CONFLICT` with the raced unique violation translated to the same, 422 `SQL_REVIEW_RULESET_INVALID` for unknown/duplicate rule ids or bad params, controller-side audit `SQL_REVIEW_RULESET_CREATED/_UPDATED/_DELETED`, PUT total at the web layer), `GET /sql-review/rules` (localized catalog, `SQL_REVIEW_MANAGE`) and the read-only `POST /sql-review/evaluate` (any user; `evaluateForUser` resolves the datasource `isAdmin ? getForAdmin : getForUser` like dry-run → 404-never-403; renders messages through `sqlreview.api.SqlReviewFindingRenderer`; no persistence, no audit row). #864 adds the enforcement — `sqlreview.api.SqlReviewFindingService` (`DefaultSqlReviewFindingService`, read-write; `recordForQuery` / `recordForGroupItem` replace the owner's `query_sql_review_findings` rows wholesale, V172 widens the table to request-group members with a `num_nonnulls = 1` CHECK) called synchronously at the three submission chokepoints (`DefaultQuerySubmissionService.submit`, `DefaultBreakGlassService.breakGlassExecute` — records, never gates — and `DefaultRequestGroupService.submit` per QUERY member); the guard lives in `workflow.internal.QueryDecisionEvaluator` as an explicit `blockingRuleIds` input (the state machine reads persisted findings at all three entry points, the access simulator evaluates live) — a `BLOCK` turns routing AUTO_APPROVE into `ROUTING_AUTO_APPROVE_SUPPRESSED` (policy still recorded, query to review), declines a covering grant, and holds the plan's own approvals; AUTO_REJECT untouched; new `SQL_REVIEW` trace step; one `SQL_REVIEW_BLOCKED` audit row (null actor, `trigger=sql_review`) only when the guard changed the outcome, also for a group whose member BLOCK forced review in `GroupAiAnalysisListener`; reviewer surfaces `sql_review_findings` (query detail, break-glass, group item) and `sql_review_blocking_count` (review queue), rendered per locale; no new NotificationEventType. #865 adds the web UI — live editor lint (`@codemirror/lint` diagnostics + findings strip, debounced `POST /sql-review/evaluate`, Submit never disabled by a BLOCK), the `/admin/sql-review` rulesets page (catalog-driven severity table, sparse rule rows), the datasource `environment` select, and the reviewer surfaces (`sql_review_findings` card on query detail, `sql_review_blocking_count` pill on the review queue). #866 is the docs/website sweep and adds [`docs/19-sql-review.md`](docs/19-sql-review.md) — read that chapter, then `docs/05-backend.md` → "Deterministic SQL review rules", before touching a rule
│   ├── api/
│   └── internal/   # rules/ (SPI + catalog), evaluator, services, renderer, params codec/validator, persistence, web
├── schemachange/   # Governed DDL change sets (epic #870): author once, promote along a `deploygov` pipeline's environment ladder as ordered `requestgroups` groups, and a scheduled schema-drift job. #878 lands the persistence foundation — V178 creates the pg enums `schema_change_set_status` / `schema_change_promotion_status` / `schema_drift_baseline` / `schema_drift_finding_kind` / `schema_drift_finding_status` and the `schema_change_sets` → `schema_change_set_statements` / `schema_change_set_promotions` and `schema_drift_scans` → `schema_drift_findings` tables (intra-module `ON DELETE CASCADE`; every cross-module ref — org, pipeline, environment, datasource, request group, user — is a bare UUID with no FK, the `deploygov` convention); a partial unique index keeps at most one non-terminal (`PENDING`/`IN_REVIEW`/`APPROVED`) promotion per `(change_set_id, environment_id)` and `SchemaChangePromotionStatus.isTerminal()` is its one code-side mirror; `statements_checksum` is `CHAR(64)` (SHA-256 hex) mapped with `@JdbcTypeCode(SqlTypes.CHAR)` — a plain `String` fails `ddl-auto=validate` against `bpchar`; the `SCHEMA_CHANGE_MANAGE` permission (WORKFLOW_ADMIN, ADMIN-only, seeded by V179). #879 adds the authoring half — `DefaultSchemaChangeSetService` behind `/api/v1/schema-change-sets` (`SCHEMA_CHANGE_MANAGE`; org-scoped, a foreign change set or pipeline reads 404-never-403; `(organizationId, changeSetId)` parameter order) and the validation gate `SchemaChangeStatementGate`: targets = every environment of the pipeline that binds a datasource (all rungs, not just the entry rung; none + statements → 409 `SCHEMA_CHANGE_SET_NO_TARGET_DATASOURCE`), JDK-only envelope / multi-statement pre-checks (`SchemaChangeStatementScanner` — proxy's marker scanner is module-private), `proxy.api.QueryParser` per distinct target `DbType`, **gate rule (a): refuse `SELECT`/`INSERT`/`UPDATE`/`DELETE`, admit `DDL` and `OTHER`** (a strict `== DDL` gate would refuse `V91`'s `ALTER TYPE … ADD VALUE`; the cost — `MERGE`/`CALL`/unrecognised text pass authoring — is documented in `docs/20-schema-change-governance.md`; never "tighten" it silently), then `sqlreview.api.SqlReviewService.evaluate` per target datasource — a `BLOCK` **refuses the save** (422 with `findings`; the one place BLOCK rejects, because a set being saved has no reviewer to escalate to), `WARN`s ride on the write view's `reviewWarnings` and are never persisted. Statements are stored normalised (trim, one trailing `;` off) and `statements_checksum` = SHA-256 hex over them joined by `\n` (null when empty) so #880 recomputes the same value. Freeze: statements and delete are refused (409 `SCHEMA_CHANGE_SET_FROZEN`) while any promotion is not `FAILED`/`CANCELLED`; `ARCHIVED` refuses statement edits; `PUT /{id}` may only move `status` to `ARCHIVED` (`ACTIVE` belongs to #880, no un-archive). Cap `accessflow.schemachange.max-statements` (50) is service-enforced (a `@Size` cannot read a property). Locale is resolved web-side through `SqlReviewFindingRenderer` + `LocaleContextHolder` (controller and `SchemaChangeExceptionHandler`); the service is locale-free. The listing is a criteria `Specification` — never a JPQL `(:status is null or …)` against the PG enum. #880 adds the promotion half — `DefaultSchemaChangePromotionService` behind `POST /schema-change-sets/{id}/promotions` (202), `GET /schema-change-sets/{id}/promotions`, `GET /schema-change-promotions/{id}` and `POST /schema-change-promotions/{id}/cancel` (204): twelve fail-closed checks in a **fixed, test-pinned order** — set 404 → archived → empty → environment on this pipeline (404 via `findEnvironment`) → bound datasource (422) → datasource still exists (409) → **`can_ddl` on the target for the promoter, no admin exemption** (403, via `core.api.DatasourceUserPermissionLookupService.findFor`) → distinct `sort_order` (409; asserted even though V177 made it unique, because over an all-equal column the ladder gate evaluates the empty set and fails open) → **the ladder gate** (every lower rung *that binds a datasource* needs an `APPLIED` promotion of this set; unbound rungs skipped) → freeze (any window refuses, `HOLD` included — an unevaluable window degrades to `HOLD`) → **review enforceability** (422 when the environment requires review but the target datasource's plan does not require human approval — `GroupReviewPlanResolver` reads the datasource plan only, so a plan-less datasource would auto-approve) → open-promotion conflict (pre-check + the raced `uq_schema_change_set_promotions_open` violation translated to the same 409). The row is flushed **before** `createDraft` so a lost race never orphans a group; the group is one `QUERY` member per statement in authored order (`RequestGroupItemInput.sequenceOrder` is ignored downstream), `continueOnError=false`, created and submitted with **`admin=true` deliberately** — `DefaultRequestGroupService.validatePermission` exempts `QUERY_ADMIN` and would demand `can_write` for an `OTHER` statement, so schemachange owns the authorization decision; never "fix" this to `admin=false`. Submitted with **`scheduledFor = clock.instant()`** so the existing `ScheduledGroupRunJob` is the durable execution trigger (approval alone never executes a group; an event-driven `execute` would be lost on a crash — there is no event-publication registry). Cost: an `APPROVED` promotion is no longer cancellable. `SchemaChangePromotionStatusListener` projects `requestgroups.events.RequestGroupStatusChangedEvent` (`PENDING_REVIEW→IN_REVIEW`, `EXECUTED→APPLIED`, `PARTIALLY_EXECUTED→PARTIALLY_APPLIED`, `REJECTED`/`TIMED_OUT`/`CANCELLED→CANCELLED`; others ignored) as `@Async @Transactional(REQUIRES_NEW) @TransactionalEventListener(AFTER_COMMIT, fallbackExecution=true)` — **not** `@ApplicationModuleListener`, because `GroupExecutionService` publishes the execution transitions with no transaction and a plain after-commit listener would strand the promotion at `APPROVED`; monotonic (terminal is final) under `findByRequestGroupIdForUpdate`, since events reorder. On `APPLIED` it stores the `introspectSchemaForSystem` snapshot (`PROMOTION_SNAPSHOT` baseline for #881; a failure keeps the transition and leaves the snapshot null). Audit: `SCHEMA_CHANGE_PROMOTION_SUBMITTED`/`_CANCELLED` carry the actor, `_APPLIED`/`_PARTIALLY_APPLIED`/`_FAILED` are null-actor rows with `trigger=request_group`; no migration (`VARCHAR(100)`). No notifications until #882. Read `docs/20-schema-change-governance.md` before touching the gate or the promotion path. #881 adds the **drift half**: `schema_drift_configs` (V180) is an opt-in row per pipeline that `SchemaDriftJob` *drains* — `deploygov` exposes no cross-organization pipeline or environment listing, so an opt-in row is the only thing that makes a scheduled scan discoverable, and `enabled` defaults FALSE because drift opens connections to customer databases on a timer (the `discovery_scan_config` precedent). Applicability is an **allow-list** of catalog-backed `DbType`s on `SchemaDriftScanService` — never `QueryEngineCatalog.isEngineManaged` (Cassandra, Elasticsearch, Snowflake, BigQuery and Databricks are engine-managed *and* catalog-backed), and five engines sample, not the two V178's comment names (MongoDB, Redis, Couchbase, DynamoDB, Neo4j — `docs/03-data-model.md` is the authority). `@SchedulerLock(name="schemaDriftJob")` guards the tick and a `schemaDriftScan:<envId>` lock guards each environment; the manual path **commits** its scan row via `SchemaDriftScanStore.open` (REQUIRES_NEW) before handing off, since `runLockedAsync` starts the executor immediately, and its in-flight 409 pre-check is bounded by the lock ceiling so a dead replica's orphan row does not block an environment forever (404 is always resolved before 409). `SchemaDriftDiffer` is pure and **never descends past an absence** (a dropped 400-column table is one finding, not 401) with one finding per `(path, kind)` and the table cap over the **union** of both sides; foreign keys are suppressed scan-wide when one side reports none at all, because the JDBC introspector swallows a failed `getImportedKeys`. Both live baseline modes require the **same `DbType`** or every column mismatches forever, the baseline is resolved **before** the target is contacted, and an unresolved baseline records zero findings **with a reason** and resolves nothing; `PROMOTION_SNAPSHOT` uses the **newest** applied promotion only (no snapshot → `BASELINE_SNAPSHOT_MISSING`, never an older snapshot). Only resolve what the scan could have re-observed (the AF-659 eligibility coupling): a finding's table is the **longest known table key prefixing its path — never split `object_path` on dots** (Elasticsearch/BigQuery nested fields and dated index names contain them), and a scan that suppressed FKs leaves `FOREIGN_KEY_MISMATCH` findings alone. Findings carry `@Version` (V181) so a scan's stale save skips rather than overwriting an acknowledgement (acknowledge → 409 `SCHEMA_DRIFT_CONCURRENT_UPDATE`). `last_scan_at`/`last_scan_error` are per pipeline, so **only `SchemaDriftScanCoordinator` stamps them, once per run** — never a single scan, never *Scan now*, and not at all when an environment was lock-skipped. `findings_count` is observed, not new, counted from the rows at finish; audit metadata keys are snake_case; `SCAN_FAILED` (AccessFlow) is kept apart from `TARGET_INTROSPECTION_FAILED` (customer DB). `core.api.SchemaFingerprintService` replaces the deleted `workflow/internal/SchemaHasher` (pre-#881 `query_snapshots.schema_hash` values came from the narrower algorithm — cosmetic, nothing compares them). **Drift never writes**. #882 adds the fan-out — `notifications` and `realtime` depend on `schemachange` (never the reverse, `requestgroups` has no notification path so a promotion notifies on its own `SchemaChangePromotionStatusChangedEvent`): `SCHEMA_CHANGE_PROMOTION_SUBMITTED` fires on `IN_REVIEW`, not submission (→ every approver rule of the datasource's plan ∪ its reviewer assignments — the `GroupReviewPlanResolver` set, since a group has one stage — else `REVIEW_OVERRIDE` holders, via `api.SchemaChangeNotificationLookupService`, never the promoter), `_APPLIED` / `_FAILED` (`PARTIALLY_APPLIED` folds in, payload `promotion_status` says which) → the promoter, and `SCHEMA_DRIFT_DETECTED` → `SCHEMA_CHANGE_MANAGE` holders, published by the scan as `events.SchemaDriftDetectedEvent` **only for findings it opened** (any transition into `OPEN` — created, reopened from `RESOLVED`, or an `ACKNOWLEDGED` finding whose values changed — reported by the reconciler through a callback so a part-way failure still counts; a finding re-seen while already open never notifies) and consumed with `fallbackExecution` because the scan holds no transaction; all four fan out org-wide, never page, never ticket; `user_notifications.schema_change_promotion_id` (V182 widens `chk_user_notifications_target`); `schema_change_promotion.status_changed` WS push to the promoter; audit adds `SCHEMA_CHANGE_PROMOTION_APPROVED` (system row) and `SCHEMA_CHANGE_SET_CREATED/_UPDATED/_STATEMENTS_REPLACED/_DELETED` (resource `schema_change_set`, so `SchemaChangeSetService.update/replaceStatements/delete` take an `actorId`)
│   ├── api/
│   ├── events/
│   └── internal/   # config, persistence (entity, repo), gate + services, scheduled, web
├── serviceaccounts/ # First-class non-human identities (epic #867). #868 lands the foundation only — PG enum principal_type (HUMAN/SERVICE_ACCOUNT) + users.principal_type (DEFAULT HUMAN, V173 backfills the bootstrap_state SERVICE_ACCOUNT rows), the 1:1 service_accounts detail table (PK user_id → users, real FKs; managed_by UI/BOOTSTRAP; mcp_tool_allow_list TEXT[] where NULL = all tools and '{}' = none; rate limits; nothing reads them until #872/#873), api contracts ServiceAccountLookupService (read) + ServiceAccountProvisioningService.ensureRegistered (the ONLY chokepoint that flips the discriminator — it calls core's UserAdminService.setPrincipalType — PrincipalTypeChokepointTest (ArchUnit) fails the build on any other caller — and upserts the detail row in one transaction, re-asserting managed_by and never touching the UI-owned fields, so a UI edit survives a bootstrap re-run), McpToolName (the twelve @Tool names — here, not mcp/api, so mcp → serviceaccounts and never the reverse; McpToolNameParityTest pins it), and the SERVICE_ACCOUNT_MANAGE permission (USERS, ADMIN-only, V174). ServiceAccountReconciler calls ensureRegistered(BOOTSTRAP) inside its changed branch only, so an unchanged spec still short-circuits; toolAllowList/rateLimit stay OUT of ServiceAccountSpec and the fingerprint. security must never depend on serviceaccounts (it reads principalType off core.api.UserView). #869 adds the two halves the later sub-issues build on — `security.api.ApiKeyAuthentication.apiKeyId()` (the filter resolves via `ApiKeyService.resolve` → `ResolvedApiKey(apiKeyId, userId)`; `resolveUserId` is a `default` over it; the key id rides on the token BESIDE `JwtClaims`, which stays byte-for-byte unchanged — `JwtClaimsTest` pins its component list), and the interactive sign-in block: `ServiceAccountSignInException` (401 `SERVICE_ACCOUNT_SIGN_IN_BLOCKED`) from `LocalAuthenticationService.login` (before the password check) / `refresh` / `issueForUser`, the SAML + OAuth2 success handlers redirecting with the same code (on `core.api.ServiceAccountUserException` from `findOrProvision`, thrown AHEAD of the LOCAL-conflict rule so a bootstrap bot never reports a conflict) before group sync / exchange code / audit, and password reset silently skipping service accounts. A service account's API key is its only credential. No UserRoleType.AGENT — ever. #871 adds the admin surface — `api.ServiceAccountAdminService` / `DefaultServiceAccountAdminService` behind `/admin/service-accounts` (`SERVICE_ACCOUNT_MANAGE`; list/get/create/update/deactivate + key issue/rotate/revoke on the account's behalf): create mints the user via `UserAdminService.createUser` with an unusable hash, defaults the role to `READONLY` (never the reconciler's `ADMIN`), then `ensureRegistered(UI)`; on a `BOOTSTRAP` account a *changed* declared field (display name / role) is 409 `SERVICE_ACCOUNT_BOOTSTRAP_MANAGED` (an equal value is a no-op; `active` and every UI-owned field stay editable; every update field is null-means-unchanged, UI-owned resets go through the `clear` list / `ServiceAccountClearableField`, so an omitted allow-list never widens); rotation issues the replacement and *expires* the old key after a grace (`accessflow.serviceaccounts.rotation-grace`, default PT24H, or a per-request `grace_period`) via `ApiKeyService.expireAt` — never an instant revoke; the bootstrap-declared key (`api_keys.bootstrap_declared`, V175, set by `importOrUpdate` which also clears it on siblings) can be neither revoked nor rotated — guarded at `DefaultApiKeyService.revoke` too, because the account can reach `/me/api-keys` with its own key and `importOrUpdate` resurrects a revoked declared key on the next changed reconcile; audit is controller-side (`SERVICE_ACCOUNT_*` against resource `service_account`), never carrying a raw key. Known limitation: `PUT /admin/users/{id}` does not consult `principal_type`; the users page (#875) badges service accounts, filters by `?principal_type=`, and routes their row action to `/admin/service-accounts/{id}` instead of the edit modal, so the UI never offers the unguarded path. #872 adds the enforcement — `api.ServiceAccountToolPolicyService` / `DefaultServiceAccountToolPolicyService` (unknown name → denied before any lookup; no detail row or NULL list → allowed; `'{}'` → nothing; else listed), consulted by `mcp.internal.tools.GuardedToolCallback` around every callback on every `tools/call`; `tools/list` stays unfiltered by SDK design (the stateless list handler ignores the transport context), so document the allow-list as an enforcement boundary, never a discovery filter. #873 adds the per-identity rate limit — `internal.ServiceAccountRateLimiter` / `DefaultServiceAccountRateLimiter` (the `DefaultAiRateLimiter` Redis idiom on two fixed windows, minute then day; limits from the `service_accounts` row when set, else `ServiceAccountRateLimitProperties` at `accessflow.serviceaccounts.rate-limit.*`, 120/min and 0/day; NULL on the row = the deployment default, so a human's personal key is capped too) enforced by `internal.web.ApiKeyRequestFilter` on EVERY `security.api.ApiKeyAuthentication` request (`/mcp/**` and `/api/v1/**`, never a JWT session) — a plain `OncePerRequestFilter`, NOT a `@Component`, registered by `ServiceAccountsConfiguration` through a `FilterRegistrationBean` at order 0 (Spring Security's proxy is at -100, so it runs inside the chain after authentication + authorization; registering it here rather than in `SecurityConfiguration` is what keeps `security → serviceaccounts` off the graph); it writes the RFC 9457 `429 SERVICE_ACCOUNT_RATE_LIMIT_EXCEEDED` itself (`MessageSource` + `request.getLocale()`, `limit` / `retryAfterSeconds` / `timestamp`, real `Retry-After` header). It **fails OPEN** on `DataAccessException` (throttled WARN, request allowed) — a deliberate divergence from `DefaultAiRateLimiter`: a rate limiter is a resource guardrail, not authorization, and failing closed would halt every agent and CI pipeline on a Redis blip. Do not "fix" it. Every CI wrapper (gate/outcome + provision-datasource/run-query actions, the GitLab hidden jobs, Azure) and the Terraform provider's HTTP client treat 429 as retryable honouring `Retry-After` (bounded: the wrapper's own timeout / `retry-timeout`, five attempts and a 2-minute per-wait cap in Go); the fake-curl harness under `.github/actions/tests/` scripts headers via `headers/<n>` sidecars and runs the extracted GitLab/Azure bodies too, so keep a new wrapper call site behind its retry helper and add a scenario. #874 adds on-behalf-of attribution — an API-key request may carry `X-AccessFlow-On-Behalf-Of: <uuid|email>` naming the human the agent acts *for*; resolved in the same `ApiKeyRequestFilter` (after the 429 check) through `internal.DefaultOnBehalfOfResolver` against `service_account_delegated_principals` (V176; a human's *consent to be named*, granted at `/me/service-account-delegations` or by a `SERVICE_ACCOUNT_MANAGE`/`USER_MANAGE` admin at `/admin/service-accounts/{id}/delegated-principals` via `api.ServiceAccountDelegationService`; soft-revoked, one live row per pair) and parked in a request attribute — **never on the `Authentication`**, so `JwtClaims` and the permission set stay the key owner's; the only reader is `api.OnBehalfOfPrincipalService`, consumed by the submission controllers (`QuerySubmissionController`, `BreakGlassController`, `QueryReplayController`, `ApiRequestController`, `DeploymentRequestController`, `RequestGroupController` draft + submit) and `McpToolService.submitQuery` to stamp `on_behalf_of_user_id` on `query_requests` / `api_requests` / `deployment_requests` (+ `deployment_rollback_reviews`) / `request_groups`, and by `internal.ServiceAccountProvenanceContributor` (the one `audit.api.AuditMetadataContributor`, merged by `DefaultAuditLogService` before hashing — explicit keys win, a throwing contributor never loses the row, empty off-thread) to put `on_behalf_of_user_id` / `api_key_id` / `service_account` on every audit row of the request. Any miss is one opaque `403 ON_BEHALF_OF_NOT_PERMITTED` (`reason=not_api_key|not_permitted` — never silently dropped); every review / decision path (`internal.web.OnBehalfOfDecisionPaths`) refuses it with `403 ON_BEHALF_OF_REVIEW_FORBIDDEN`, and `GuardedToolCallback` refuses `review_query` whenever a principal is present. The named human is a **second submitter identity** for the self-approval ban (`isSubmitterIdentity` in the four review services + the rollback review, the delegated-candidate exclusions and the four queue filters) — the approval-laundering hole the epic closes — while `*_review_decisions.on_behalf_of_user_id` (#622 provenance, confers eligibility) is never written from it; the reverse direction is barred too (`DefaultReviewDelegationService` refuses a non-`HUMAN` delegate). Break-glass retro-review acknowledgement keeps the submitter-only guard (follow-up)
│   ├── api/
│   └── internal/   # persistence, Default* services
...
│   └── internal/
└── mcp/            # Spring AI stateless MCP server — @Tool callbacks for AI agents; every callback is wrapped in `GuardedToolCallback` (#872), which returns the documented `{"code":"permission_denied"}` when `serviceaccounts.api.ServiceAccountToolPolicyService` denies the tool — safe to read the SecurityContext there only because the stateless auto-config sets `immediateExecution(true)` in a servlet environment (pinned by `McpToolAllowListIntegrationTest`)
    ├── api/
    └── internal/
```
...
- `query_requests.status` transitions must follow the state machine exactly:
  ...
  PENDING_AI → PENDING_REVIEW (SQL review BLOCK — #864; any BLOCK finding recorded at submission
                          suppresses EVERY PENDING_AI → APPROVED auto path above except break-glass:
                          routing AUTO_APPROVE (policy still recorded on routing_decision), the
                          grant fast path, and the plan's requires_human_approval=false /
                          auto_approve_reads. Never rejects — AUTO_REJECT still rejects. Audited
...
| @codemirror/lint | 6.x | One diagnostics surface for live deterministic SQL review findings and AI issues — lint gutter + hover tooltips, pushed by `setDiagnostics` without rebuilding the view (#865) |
...
├── layouts/      # AppLayout — the only layout; admin and auth pages style themselves
