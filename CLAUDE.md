...
├── engines/          # Engine plugins — engines/mongodb/, engines/couchbase/ (standalone Maven projects, shaded JARs)
...
- `engines` — installs the backend plain jar, builds every `engines/*` plugin (unit + Testcontainers tests; mongodb, couchbase, and redis today), and fails when a shaded JAR's SHA-256 drifts from the pin in its `connectors/<id>/connector.json`. Runs when `engines/**`, `backend/**`, or `connectors/**` (or the workflow file) changed.
