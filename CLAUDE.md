...
├── engines/          # Engine plugins — engines/mongodb/, engines/couchbase/, engines/cassandra/ (standalone Maven projects, shaded JARs)
...
- Adapters route their HTTP calls through **Spring AI 2.0** (`spring-ai-bom:2.0.0-RC2`). The autowired `AiAnalyzerStrategy` is `AiAnalyzerStrategyHolder` — it builds `AnthropicChatModel` / `OpenAiChatModel` / `OllamaChatModel` per-org from the `ai_config` row, caches the delegate, and evicts on `AiConfigUpdatedEvent` (no Spring context refresh, no restart).
- The same Spring AI BOM also pins `spring-ai-starter-mcp-server-webmvc` (`2.0.0-RC2`) — the stateless MCP server starter used by the `mcp` module. The starter ships with `spring-ai-autoconfigure-mcp-server-common` as an explicit dependency in `backend/pom.xml` to work around a missing transitive in the RC2 starter POM (`McpServerStdioDisabledCondition` lives in the common artifact). See `docs/13-mcp.md` and `docs/05-backend.md` → "MCP server and user API keys".
...
- `engines` — installs the backend plain jar, builds every `engines/*` plugin (unit + Testcontainers tests; mongodb, couchbase, redis, and cassandra today — the cassandra JAR backs both the `cassandra` and `scylladb` connectors), and fails when a shaded JAR's SHA-256 drifts from the pin in its `connectors/<id>/connector.json`. Runs when `engines/**`, `backend/**`, or `connectors/**` (or the workflow file) changed.
