...
| `website-drift.sh` | `Write\|Edit` on `website/**.html` | Stale `sitemap.xml` `<lastmod>` and JSON-LD `dateModified` — `websitePages.test.ts` checks the three date copies agree, never that they are current, and this is that missing half. Plus retired `HowTo`/`FAQPage` schema. |
