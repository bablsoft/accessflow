...
**Authoritative semantics.** When `ACCESSFLOW_BOOTSTRAP_ENABLED=true`, the backend reconciles every declared row on each restart. Rows that match a declared spec are **overwritten**; rows not declared are untouched. Note the reconciler short-circuits on an unchanged spec (see *Auditability* below), so an admin-UI edit to a declared resource is **not** reverted by a restart alone — it is overwritten the next time the declared values themselves change. Operators should still treat the env-driven set as the source of truth, since any later bootstrap change silently discards the UI edit.

Each section also carries its own enable flag: `ACCESSFLOW_BOOTSTRAP_ENABLED=true` alone is not sufficient. A section whose flag is unset (for example `ACCESSFLOW_BOOTSTRAP_SYSTEM_SMTP_ENABLED`) is skipped with a debug-level log and no error.
...
| `ACCESSFLOW_COMPLIANCE_RESULT_EXPORT_MAX_ROWS` | Optional | `50000` | Hard cap on rows in a result-set CSV export (`GET /queries/{id}/results/export`, #626); a smaller policy `row_cap` wins. Beyond it the export sets `X-AccessFlow-Export-Truncated: true`. |
| `ACCESSFLOW_COMPLIANCE_RESULT_EXPORT_MAX_PDF_ROWS` | Optional | `5000` | Hard cap on rows in a result-set PDF export (#626) — PDF table rendering of huge result sets is expensive; the CSV cap governs everything else. |
| `ACCESSFLOW_POLICY_SIMULATION_MAX_ROWS` | Optional | `5000` | Hard cap on historical `query_requests` rows one policy simulation (AF-630) replays, newest first; beyond it the result sets `truncated=true` and every count is over the evaluated subset only. Far below the compliance cap because each row is re-parsed and evaluated **twice** — once against the current policy set, once with the draft applied. |
| `ACCESSFLOW_POLICY_SIMULATION_MAX_SAMPLES` | Optional | `100` | Hard cap on drill-down sample rows a policy simulation returns. The counts are the answer; the samples are the detail (and carry no SQL text). |
| `ACCESSFLOW_POLICY_SIMULATION_MAX_USER_IMPACTS` | Optional | `100` | Hard cap on entries in a policy simulation's per-user impact list, ordered by changed-query count descending. |
| `ACCESSFLOW_POLICY_SIMULATION_MAX_WINDOW` | Optional | `P90D` | ISO-8601 duration. Longest period a single policy simulation may span; a longer `from`/`to` window is rejected with `400 INVALID_SIMULATION_PERIOD`. |
...
| `ACCESSFLOW_RAG_PGVECTOR_DIMENSIONS` | Optional | `1536` | Embedding dimension of the in-app pgvector `vector_store` column. **Set before the first migration** — it fixes the `vector(N)` column type (Flyway V69 placeholder). Must match the embedding model's output dimension (e.g. 1536 for OpenAI `text-embedding-3-small`, 768 for Ollama `nomic-embed-text`). **Voyage AI cannot use the default:** its models emit 256 / 512 / 1024 / 2048 and never 1536, so an install that intends to embed with Voyage must either set this to one of those widths before its first migration, or pair Voyage with an external Qdrant store. AccessFlow refuses the impossible combination when the AI configuration is saved rather than when ingestion first runs. |
...
- **Unavailable** (or `ACCESSFLOW_RAG_PGVECTOR_ENABLED=false`) → V69 is recorded as applied without executing it, `knowledge_document` is created by the idempotent `V73` migration, the `vector_store` table is omitted, and the in-app PGVECTOR store is disabled. A `WARN` is logged, `GET /admin/ai-configs/rag/capabilities` reports `pgvector_available: false`, the admin UI shows a banner on the RAG settings, and PGVECTOR ingestion / connection tests return `400 RAG_CONFIG_INVALID`. Enabling the in-app help agent with retrieval on a PGVECTOR configuration is refused the same way (`400 HELP_AGENT_CONFIG_INVALID`) — and it names *which* of the two cases you are in, since installing the extension and clearing `ACCESSFLOW_RAG_PGVECTOR_ENABLED=false` are different fixes. External Qdrant RAG continues to work. Install the extension (or use a pgvector-enabled image) and restart to enable PGVECTOR.
...
| `ACCESSFLOW_PROXY_MONGO_MAX_POOL_SIZE` | Optional | `10` | Max connections in the native MongoDB driver's internal pool. Legacy alias for `accessflow.proxy.engines.mongodb.max-pool-size` |
| `ACCESSFLOW_PROXY_ENGINES_COUCHBASE_CONNECT_TIMEOUT` | Optional | `PT10S` | Connect timeout for the per-Couchbase-datasource native `Cluster` (Couchbase-only; relational pools use `ACCESSFLOW_PROXY_CONNECTION_TIMEOUT`). Generic AF-418 lane — no legacy alias |
| `ACCESSFLOW_PROXY_ENGINES_COUCHBASE_WAIT_UNTIL_READY_TIMEOUT` | Optional | `PT10S` | How long a freshly opened Couchbase cluster waits for the datasource bucket to be ready before the first query |
| `ACCESSFLOW_PROXY_ENGINES_COUCHBASE_SCAN_CONSISTENCY` | Optional | `request-plus` | SQL++ scan consistency: `request-plus` (reads observe mutations submitted before the query — the predictable default for a governance proxy) or `not-bounded` (Couchbase's own default, faster under load) |
...
| `ACCESSFLOW_PROXY_ENGINES_ELASTICSEARCH_CONNECT_TIMEOUT` | Optional | `PT10S` | Connect timeout for the per-Elasticsearch-datasource low-level REST client (Elasticsearch-only; relational pools use `ACCESSFLOW_PROXY_CONNECTION_TIMEOUT`). Generic AF-418 lane — no legacy alias |
...
| `ACCESSFLOW_PROXY_ENGINES_DATABRICKS_RESULT_DISPOSITION` | Optional | `auto` | Statement Execution API result disposition (#633). `auto` submits `INLINE` and falls back once to `EXTERNAL_LINKS` when the API's ~25 MiB inline ceiling is hit, for side-effect-free statements only; `inline` or `external-links` force a single mode. Unrecognised values fall back to `auto` |
| `ACCESSFLOW_PROXY_ENGINES_DATABRICKS_MAX_RESULT_BYTES` | Optional | `52428800` | Engine-side byte backstop while streaming `EXTERNAL_LINKS` result chunks (#633); on exceed the result is truncated with `truncated_reason=BYTE_LIMIT`. Clamped to 1 MiB–1 GiB. Independent of `ACCESSFLOW_PROXY_EXECUTION_MAX_RESULT_BYTES`, which governs the relational JDBC path |
...
| `ACCESSFLOW_WORKFLOW_ESCALATION_POLL_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. Cadence at which `ReviewEscalationJob` (#622) escalates `PENDING_REVIEW` queries past their plan's `escalation_after_hours` and re-nudges undecided reviewers on `nudge_interval_hours`. Notify-only — it never changes who may approve. ShedLock makes this safe under horizontal scaling. |
| `ACCESSFLOW_SCHEDULING_ENABLED` | Optional | `true` | Master switch for Spring's `@EnableScheduling`. When `false`, **no `@Scheduled` job in any module runs** — timeouts, escalations, scheduled and recurring query runs, retention, erasure, attestation and deployment jobs all stop. ShedLock's `@SchedulerLock` advice stays wired either way (`SchedulerLockConfiguration` is separate and unconditional). Note: Spring Modulith's moments auto-configuration currently carries its own `@EnableScheduling`, so in an eagerly-initialised context this switch does not stop jobs on its own (tracked as a follow-up); the **Scheduled jobs** page shows what is actually scheduled. Leave it `true` in every real deployment; it exists so the integration suite, which shares one long-lived Spring context across ~90 test classes, does not have those jobs mutating the shared test database underneath assertions. |
| `ACCESSFLOW_SCHEDULING_EXECUTIONS_ENABLED` | Optional | `true` | Record every `@Scheduled` run that actually held its ShedLock lock in `job_executions` (#923), shown on the platform-admin **Scheduled jobs** page. Lock-skipped ticks are never written, and recording is fail-soft — it never changes a job's outcome. `false` stops recording; the job registry still works. |
| `ACCESSFLOW_SCHEDULING_EXECUTIONS_RETENTION` | Optional | `P14D` | ISO-8601 age after which `JobExecutionRetentionJob` deletes execution rows. A non-positive value skips age pruning. |
| `ACCESSFLOW_SCHEDULING_EXECUTIONS_MAX_PER_JOB` | Optional | `500` | Each job keeps at most this many newest execution rows, so a `PT30S` job cannot crowd out a daily job's history. A non-positive value skips the cap. |
| `ACCESSFLOW_SCHEDULING_EXECUTIONS_RETENTION_POLL_INTERVAL` | Optional | `PT6H` | ISO-8601 cadence of `JobExecutionRetentionJob`. |
| `ACCESSFLOW_SCHEDULING_EXECUTIONS_SUMMARY_WINDOW` | Optional | `PT24H` | ISO-8601 window the per-job success/failure counts and mean duration on the **Scheduled jobs** page cover. A non-positive value falls back to `PT24H`. |
| `ACCESSFLOW_APIGOV_ESCALATION_POLL_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. As above for `ApiReviewEscalationJob`, scanning `api_requests` against their connector's review plan. |
| `ACCESSFLOW_DEPLOYGOV_TIMEOUT_CHECK` | Optional | `PT5M` | ISO-8601 duration. Cadence at which `DeploymentTimeoutJob` (the `deploygov` module, #692) auto-rejects `PENDING_REVIEW` deployment requests older than their resolved review plan's `approval_timeout_hours` to `TIMED_OUT` (environment plan override wins over the pipeline's; a request whose pipeline/environment resolves no plan never times out). ShedLock-guarded. |
| `ACCESSFLOW_DEPLOYGOV_RELEASE_CHECK` | Optional | `PT1M` | ISO-8601 duration. Cadence at which `ScheduledDeploymentReleaseJob` (the `deploygov` module, #693) scans `APPROVED` deployment requests that became releasable — a `HOLD` freeze window ended or `scheduled_for` arrived — and announces each one exactly once (`release_notified_at` latch) via `DeploymentReleasableEvent`, so push-style integrations need not tight-poll the gate. The gate endpoint stays the source of truth. ShedLock-guarded. |
| `ACCESSFLOW_SCHEMACHANGE_MAX_STATEMENTS` | Optional | `50` | Hard cap on statements per schema change set (the `schemachange` module, #879); a create or statement replacement beyond it returns HTTP 400 `SCHEMA_CHANGE_SET_STATEMENT_LIMIT`. A cost control rather than a style rule: promotion (#880) submits every statement as a request-group member that is AI-analysed and re-introspects the datasource, so the cap bounds AI-provider calls and customer-database round-trips per change set and per environment. `0` or a negative value falls back to the default. The web UI's statement editor mirrors the default (it stops offering *Add statement* at 50); a raised cap still applies to API and Terraform clients, and a lowered one surfaces as the server's 400 in the UI (#883). |
| `ACCESSFLOW_SCHEMACHANGE_DRIFT_POLL_INTERVAL` | Optional | `PT6H` | ISO-8601 duration. Cadence at which `SchemaDriftJob` (the `schemachange` module, #881) checks for pipelines whose schema drift scan is due (`enabled` rows in `schema_drift_configs` past their own `scan_interval_hours`). ShedLock-guarded. Drift is **opt-in per pipeline and off by default**, so this job does nothing until an admin enables it — see [20-schema-change-governance.md](20-schema-change-governance.md#7-schema-drift-881). A pipeline's own `scan_interval_hours` shorter than this poll behaves like the poll. |
| `ACCESSFLOW_SCHEMACHANGE_DRIFT_SCAN_TIME_BUDGET` | Optional | `PT5M` | ISO-8601 duration. Wall-clock budget for one environment's diff loop. Tables past the deadline are skipped and the scan is flagged `partial` — and because a skipped table resolves nothing, a scan that runs out of time is incomplete rather than wrong. Both schema introspections are charged against the same deadline, so two slow databases simply leave the loop less budget. `0` or a negative value falls back to the default. |
| `ACCESSFLOW_SCHEMACHANGE_DRIFT_MAX_TABLES_PER_SCAN` | Optional | `500` | Hard cap on tables compared in one drift scan, applied to the **union** of the baseline's and the scanned environment's tables; beyond it the scan is flagged `partial`. Tables under a schema that is missing entirely do not count, since a missing schema is reported once and never descended into. `0` or a negative value falls back to the default. |
| `ACCESSFLOW_SCHEMACHANGE_DRIFT_MAX_FINDINGS_PER_SCAN` | Optional | `500` | Hard cap on findings recorded by one drift scan; beyond it the scan is flagged `partial`. Bounds the damage a genuinely divergent pair of databases can do to the worklist. `0` or a negative value falls back to the default. |
| `ACCESSFLOW_SCHEMACHANGE_DRIFT_SCAN_LOCK_AT_MOST_FOR` | Optional | `PT30M` | ISO-8601 duration. How long the cluster-wide `schemaDriftScan:<environmentId>` lock may be held, which is what stops an on-demand *Scan now* on one replica and `SchemaDriftJob` on another from scanning the same environment at once. A crash ceiling rather than a timeout — the lock is released as soon as the scan ends, so this only bounds how long a dead node keeps an environment locked out. Always raised to at least `2 × ACCESSFLOW_SCHEMACHANGE_DRIFT_SCAN_TIME_BUDGET + 10m`, since a lock that lapses under a still-running scan admits the second scanner it exists to keep out — the budget bounds the table loop, not the whole run, so the additive term is what covers the two introspections and the reconciliation around it. It also bounds the API's in-flight check, so a scan row orphaned by a crashed replica stops answering `409 SCHEMA_DRIFT_SCAN_IN_PROGRESS` once its lock could no longer be held. To clear a claim left behind by a crashed replica instead of waiting it out, delete the Redis key `job-lock:accessflow:shedlock:schemaDriftScan:<environmentId>`. |
| `ACCESSFLOW_REQUESTGROUPS_ESCALATION_POLL_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. As above for `GroupReviewEscalationJob`. A bundle's window is the **minimum** non-null `escalation_after_hours` across its members' plans. Records `escalated_at` on the bundle and nothing more: no notification (the `requestgroups` module has no notification path yet) and no queue flag. There is no group nudge half at all, for the same reason — `request_groups` has no `last_nudged_at`. |
...
| `ACCESSFLOW_WORKFLOW_RECURRENCE_MIN_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. Minimum gap between two occurrences of a recurring query series (#627); submissions with a shorter cadence are rejected with HTTP 400. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_ENABLED` | Optional | `true` | Master switch for automatic query suggestions (#776). Advisory and provider-free, hence on by default. When `false` the aggregation job no-ops and the rail returns an empty list — an unknown datasource still answers 404, so a disabled install does not leak the difference between "off" and "not yours". |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_AGGREGATION_POLL_INTERVAL` | Optional | `PT6H` | ISO-8601 duration. Cadence at which `QuerySuggestionAggregationJob` re-mines each organisation's approved query history. The job's `lockAtLeastFor` is 5 minutes, so a value below that is effectively rounded up to it. ShedLock makes it safe under horizontal scaling. The timer is per-replica and starts at context refresh, so a multi-replica cluster may rebuild more than once per period; the aggregation is deterministic, so extra passes rewrite the same rows. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_LOOKBACK` | Optional | `P90D` | ISO-8601 duration. How far back the approved-query corpus reaches. A query shape whose last approval falls outside the window is swept on the next pass. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MAX_CORPUS_ROWS_PER_DATASOURCE` | Optional | `2000` | Newest approved rows read per datasource in one pass. Per datasource rather than per organisation: an org-wide cap would be spent almost entirely on the busiest datasource. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MAX_SUGGESTIONS_PER_DATASOURCE` | Optional | `50` | Suggestions retained per datasource, kept by frequency then recency. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MAX_TRACKED_SUBMITTERS` | Optional | `50` | Submitter ids recorded per suggestion, bounding the stored array. Used only to compute a viewer's table affinity; the reported distinct-submitter count is unaffected by the cap. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MIN_APPROVED_COUNT` | Optional | `2` | Approvals a query shape needs before it is offered, so a one-off query cannot fill the rail. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MAX_SQL_LENGTH` | Optional | `4000` | Characters. Longer approved queries are skipped rather than parsed and stored. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_RECENCY_HALF_LIFE` | Optional | `P14D` | ISO-8601 duration. Age at which the ranking's recency term halves. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_WEIGHT_FREQUENCY` | Optional | `1.0` | Weight on `ln(1 + approved_count)`. `0` switches the term off. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_WEIGHT_RECENCY` | Optional | `1.0` | Weight on the recency decay. `0` switches the term off. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_WEIGHT_TABLE_OVERLAP` | Optional | `0.5` | Weight on the Jaccard overlap between a suggestion's tables and the viewer's own recent tables. `0` makes the rail identical for every viewer who can see it. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_DEFAULT_LIMIT` | Optional | `10` | Suggestions returned when the request sends no `limit`. Clamped to the max below. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_MAX_LIMIT` | Optional | `50` | Upper bound the endpoint clamps `limit` to. Note the endpoint's own Bean Validation caps `limit` at 50, so raising this alone does not widen the API. |
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_RECOMPUTE_LOCK_AT_MOST_FOR` | Optional | `PT10M` | ISO-8601 duration. Cluster-wide per-datasource rebuild lock. Despite the name it governs **both** writers — the on-demand recompute and each datasource of the scheduled pass, which share one lock so they cannot interleave. Must exceed the worst-case rebuild for a single datasource; a lock that expires mid-run lets a second writer start a concurrent rebuild. |
...
| `ACCESSFLOW_DASHBOARD_WEEKLY_DIGEST_POLL_INTERVAL` | Optional | `P1D` | ISO-8601 duration. Cadence at which `WeeklyDigestJob` (the `dashboard` module, AF-498) wakes to scan for users opted into the weekly digest. ShedLock makes this safe under horizontal scaling. |
| `ACCESSFLOW_DASHBOARD_WEEKLY_DIGEST_PERIOD` | Optional | `P7D` | ISO-8601 duration. Minimum gap between digests for a given user — the job sends at most once per period regardless of poll cadence or restarts. |
...
| `ACCESSFLOW_APIGOV_MAX_REQUEST_BODY_BYTES` | Optional | `5242880` | Cap on the total encoded size of a submitted API request body (#517) — raw text, x-www-form-urlencoded, or the base64-decoded size of form-data / binary file parts. Files ride inline as bounded base64 (no object storage). Exceeding it rejects the submission with HTTP 422. |
| `ACCESSFLOW_APIGOV_MAX_RESPONSE_BYTES` | Optional | `10485760` | System-wide hard ceiling (#521) on a stored — and therefore downloadable — API response body; the absolute backstop above any per-connector `max_response_bytes` (effective cap = min of the two). The body lives in a Postgres `text` column and is read fully into JVM memory during the call. |
| `ACCESSFLOW_APIGOV_MAX_VARIABLE_VALUE_BYTES` | Optional | `8192` | Cap on a single resolved connector dynamic-variable value (AF-613) — request signatures, nonces, timestamps. A digest is tens of bytes, so a larger value means a runaway expression or an abusive per-request override. Exceeding it fails the execution; a submitted override over the cap is rejected with 422 at submit time. |
| `ACCESSFLOW_APIGOV_RESPONSE_PREVIEW_BYTES` | Optional | `65536` | How much of the stored response snapshot is embedded inline in the API-request detail view (#521); beyond it the full body is fetched via `GET /api-requests/{id}/response`. |
| `ACCESSFLOW_LIFECYCLE_POLICY_SCAN_INTERVAL` | Optional | `PT1H` | ISO-8601 duration. Cadence at which `RetentionPolicyScanJob` (the `lifecycle` module, AF-499) scans enabled retention policies and stages eligible `lifecycle_runs`. ShedLock-guarded. |
| `ACCESSFLOW_LIFECYCLE_POLICY_EXECUTION_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. Cadence at which `RetentionPolicyExecutionJob` (the `lifecycle` module, AF-519) drains STAGED `RETENTION_POLICY` runs and applies the action through the proxy. ShedLock-guarded. |
| `ACCESSFLOW_LIFECYCLE_ERASURE_EXECUTION_INTERVAL` | Optional | `PT1M` | ISO-8601 duration. Cadence at which `ErasureExecutionJob` (the `lifecycle` module, AF-499) executes `APPROVED` right-to-erasure requests (`APPROVED → EXECUTED`/`FAILED`). ShedLock-guarded. |
| `ACCESSFLOW_LIFECYCLE_REVIEW_TIMEOUT_POLL_INTERVAL` | Optional | `PT5M` | ISO-8601 duration. Cadence at which `ErasureReviewTimeoutJob` (the `lifecycle` module, AF-519) scans for erasure requests stuck in `PENDING_REVIEW` past the review timeout and auto-rejects them. ShedLock-guarded. |
...
| `ACCESSFLOW_DISCOVERY_SCAN_LOCK_AT_MOST_FOR` | Optional | `PT30M` | ISO-8601 duration. How long the cluster-wide `discoveryScan:<datasourceId>` lock may be held (AF-660), which is what stops an on-demand *Scan now* on one replica and `DiscoveryScanJob` on another from sampling the same datasource at once. A crash ceiling rather than a timeout — the lock is released as soon as the scan ends, so this only bounds how long a dead node keeps a datasource locked out. Always raised to at least `2 × ACCESSFLOW_DISCOVERY_SCAN_TIME_BUDGET + 10m`, since a lock that lapses under a still-running scan admits the second scanner it exists to keep out — the budget bounds the table loop, not the whole run, so the additive term is what covers introspection, the overrunning last table and the stale sweep. To clear a claim left behind by a crashed replica instead of waiting it out, delete the Redis key `job-lock:accessflow:shedlock:discoveryScan:<datasourceId>`. |
...
| `ACCESSFLOW_DISCOVERY_MAX_NESTED_DEPTH` | Optional | `5` | How far the discovery scan walks into a nested document/map cell when flattening it to dot-path pseudo-columns (AF-658). Map keys consume a level; lists fan out without adding a path segment but still cost a level. |
| `ACCESSFLOW_DISCOVERY_MAX_NESTED_LEAVES_PER_ROW` | Optional | `100` | Per-row allowance of *nested* nodes the discovery scan visits while flattening. Charged for every nested node touched (not only values kept), so a pathological document cannot turn a bounded sample into an unbounded scan. A top-level scalar cell is free whatever its type, so wide relational tables are unaffected. |
| `ACCESSFLOW_DISCOVERY_STALE_SCANS_BEFORE_EXPIRY` | Optional | `3` | How many consecutive discovery scans may sample a finding's table without re-proposing the column before the finding is retired as `STALE` and leaves the active worklist (AF-659). Re-detection resets the counter and revives the finding. Not 1 by default because the scan's sample has no `ORDER BY`, so a column near the match-ratio threshold can legitimately be missed by one run. |
...
| `ACCESSFLOW_ACCESS_USAGE_AGGREGATION_POLL_INTERVAL` | Optional | `PT1H` | ISO-8601 duration. Cadence at which `GrantUsageAggregationJob` (the `access` module, #625) reconciles the per-grant usage summaries, folds new `audit_log` execution events into them, recomputes each recommendation, and emits due staleness nudges. ShedLock makes this safe under horizontal scaling — without it every replica would fold the same window and multiply every usage count. |
| `ACCESSFLOW_ACCESS_USAGE_BACKFILL_WINDOW` | Optional | `P90D` | ISO-8601 duration (#625). How far back a newly-summarised grant starts observing. Bounds the one-time backfill read and caps how far into the past "never used" can see — a grant unused only before this window looks unused, which is the intent. |
| `ACCESSFLOW_ACCESS_USAGE_STALENESS_THRESHOLD` | Optional | `P60D` | ISO-8601 duration (#625). A grant unused for longer than this is recommended `STALE` (or `NEVER_USED` if it never was used) and becomes eligible for the `GRANT_STALE` nudge. |
| `ACCESSFLOW_ACCESS_USAGE_MIN_OBSERVATION_WINDOW` | Optional | `P14D` | ISO-8601 duration (#625). Below this the verdict is `INSUFFICIENT_DATA` rather than `NEVER_USED`, so a freshly-created grant is never recommended for revocation just for being new. Recent use still wins: a grant used today is `ACTIVE` regardless of its age. |
| `ACCESSFLOW_ACCESS_USAGE_OVER_SCOPED_THRESHOLD` | Optional | `0.5` | Exercised/granted target ratio below which an actively-used, scope-limited grant is flagged `OVER_SCOPED`. Clamped to (0, 1]. Unrestricted grants (empty allow-list) are never flagged — there is no granted scope to under-use. |
| `ACCESSFLOW_ACCESS_USAGE_MAX_ROWS_PER_TICK` | Optional | `50000` | Cap on audit events folded per organization per tick (#625). On a full page the cursor advances to the last event rather than the window end, so the undrained tail is picked up next tick instead of being skipped. |
| `ACCESSFLOW_ACCESS_USAGE_MAX_PAGES_PER_TICK` | Optional | `20` | Keyset pages read per organization per tick by `GrantUsageAggregationJob`. The read drains until a short page arrives; this bounds the work one very busy tenant can do in a single tick. Hitting it is logged and the cursor stops at the last applied event, so the remainder is picked up next tick rather than skipped. |
| `ACCESSFLOW_ACCESS_USAGE_MAX_TRACKED_TARGETS` | Optional | `200` | Cap on the distinct exercised tables/operations retained per grant (#625). A cap hit is logged, never silently truncated. |
| `ACCESSFLOW_ACCESS_USAGE_MAX_REPORT_ROWS` | Optional | `50000` | Hard cap on rows in a single over-provisioned access CSV export; beyond it the export is flagged truncated rather than shipping a partial inventory silently. |
| `ACCESSFLOW_ACCESS_USAGE_NUDGE_ENABLED` | Optional | `true` | Master switch for the `GRANT_STALE` staleness notification to org admins (#625). Set `false` to keep the report and the attestation evidence without any outbound nudge. |
| `ACCESSFLOW_ACCESS_USAGE_NUDGE_COOLDOWN` | Optional | `P30D` | ISO-8601 duration (#625). Minimum time between nudges for the same grant (tracked in `grant_usage_summary.nudged_at`), so the notification stays a nudge rather than an hourly drip. |
...
| `ACCESSFLOW_NOTIFICATIONS_TICKETING_SIGNATURE_TOLERANCE` | Optional | `PT5M` | ISO-8601 duration — acceptance window for the `X-AccessFlow-Timestamp` HMAC signature on inbound ServiceNow / Jira ticket-status webhooks (`/api/v1/integrations/{servicenow,jira}/webhook/{channelId}`, AF-453); also the replay-dedup window. |
...
| `ACCESSFLOW_SERVICEACCOUNTS_ROTATION_GRACE` | Optional | `PT24H` | ISO-8601 duration (#871). Default grace window when an admin rotates a service account's API key (`POST /admin/service-accounts/{id}/api-keys/{keyId}/rotate`): the replacement is issued immediately and the old key's `expires_at` is set to `now + grace` (or kept as is when its existing expiry is sooner), so a running agent or CI job keeps authenticating while the secret is swapped. A per-request `grace_period` overrides it. Rotation never revokes — a leaked key is handled by the revoke endpoint. |
| `ACCESSFLOW_SERVICEACCOUNTS_RATE_LIMIT_REQUESTS_PER_MINUTE` | Optional | `120` | Per-identity request cap per minute for **every API-key-authenticated request** — `/api/v1/**` and `/mcp/**` alike, never a JWT browser session (#873). A `service_accounts` row's own `rate_limit_per_minute` overrides it; a human's personal API key always gets this default. Redis fixed window; over the cap the API answers `429 SERVICE_ACCOUNT_RATE_LIMIT_EXCEEDED` with a `Retry-After` header, which the deployment CI wrappers honour. A value `<= 0` disables the per-minute limit. **Fails open:** when Redis is unreachable requests are served unmetered and a throttled WARN is logged — a rate limiter is a resource guardrail, not an authorization check. |
| `ACCESSFLOW_SERVICEACCOUNTS_RATE_LIMIT_REQUESTS_PER_DAY` | Optional | `0` | Per-identity daily cap on the same traffic (24 h fixed window); `0` (the default) = unlimited, a value `<= 0` disables it. Overridden per account by `service_accounts.rate_limit_per_day`. |
