...
| `ACCESSFLOW_WORKFLOW_QUERY_SUGGESTIONS_RECOMPUTE_LOCK_AT_MOST_FOR` | Optional | `PT10M` | ISO-8601 duration. Cluster-wide lock held by an on-demand recompute. Must exceed the worst-case rebuild for one datasource; a lock that expires mid-run lets a second replica start a concurrent rebuild. |
