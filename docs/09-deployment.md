...
- The AES-256-GCM encryption key and the JWT RS256 private key are generated on
  ...
  The Secret carries `helm.sh/resource-policy: keep` and the values are preserved
  across upgrades via Helm's `lookup` function.
- The PostgreSQL password is generated by the bundled bitnami subchart and stored
  in `{release}-postgresql` (key `password`).
...
# Leave existingSecret empty to let the bitnami subchart auto-generate and
# preserve the password in {release}-postgresql; set it to point at your own
# Secret (key: "password") if you manage credentials externally.
postgresql:
  enabled: true
  auth:
    database: accessflow
...
```bash
# PostgreSQL password (key MUST be "password" — bitnami chart convention)
