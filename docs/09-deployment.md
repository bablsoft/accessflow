...
| `ACCESSFLOW_DISCOVERY_MAX_NESTED_LEAVES_PER_ROW` | Optional | `100` | Per-row allowance of nested nodes the discovery scan visits while flattening. Charged for every node touched (not only values kept), so a pathological document cannot turn a bounded sample into an unbounded scan. Top-level scalar cells are free, so wide relational tables are unaffected. |
