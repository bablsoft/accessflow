...
Snowflake authenticates with a password **or** a key-pair PKCS#8 private-key PEM (encrypted or not — a passphrase-protected key stores its passphrase in `private_key_passphrase_encrypted`) in the encrypted
