...
- **Scheduled execution banner & metadata row** (AF-345) — when the query carries a `scheduled_for` timestamp the metadata sidebar renders a "scheduled" row with the formatted instant; when the status is `APPROVED` and the timestamp is still in the future, an info `Alert` at the top of the main column tells the user the query will auto-execute later and the **Cancel query** button switches its label/copy to **Cancel schedule** (the underlying `POST /queries/{id}/cancel` call is unchanged — backend extends the allowed states to include `APPROVED` with `scheduled_for`).
