...
- **CodeMirror SQL editor** — see SQL Editor section below. `SqlEditor` picks its CodeMirror language by engine: PostgreSQL/MySQL dialects for relational datasources; for a `MONGODB` datasource it highlights as **JavaScript** (shell mode) or **JSON** (JSON-command mode), driven by the `mongoSyntax` prop.
- **MongoDB query-syntax selector** — for a MongoDB datasource the toolbar shows an Ant Design `Segmented` **Shell / JSON** control (state `mongoSyntax`) in place of the SQL **Format** button (which is SQL-only). The submitted text still goes in the `sql` field; the backend auto-detects the form. Text-to-SQL is hidden for MongoDB datasources.
