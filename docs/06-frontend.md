...
- **Query-syntax selector** — when the engine mode declares more than one syntax (MongoDB: Shell / JSON), the toolbar shows an Ant Design `Segmented` control; the SQL **Format** button renders only when the mode's `canFormat` is true (SQL engines). The submitted text still goes in the `sql` field; the backend auto-detects the form. Text-to-SQL is gated on the mode's `supportsTextToSql` (false for MongoDB). The registry also supplies `defaultResultView`, which `QueryDetailPage` passes to `QueryResultsTable` (MongoDB results open in the JSON document view).
