...
| `SCHEMA_CHANGE_PROMOTION_SUBMITTED` | A schema change set promotion reaches `IN_REVIEW` — its request group entered `PENDING_REVIEW` (#882, epic #870). Deliberately **not** on submission (`PENDING`): a promotion whose environment needs no review never pings anyone | The target datasource's eligible reviewers — its review plan's stage-1 approver rules unioned with the per-datasource reviewer assignments, else all REVIEWER/ADMIN role holders — excluding the promoter. Fanned out to **all** active org channels. Never pages, never opens a ticket. | implemented |
...
| `SCHEMA_CHANGE_PROMOTION_FAILED` | A promotion's run failed — `FAILED`, or `PARTIALLY_APPLIED` (the run stops on the first error, so a partial run means a statement failed; the payload `status` says which) (#882) | The promoter. Org-wide channel fanout; never pages, never opens a ticket. | implemented |
| `SCHEMA_DRIFT_DETECTED` | A schema-drift scan of one environment **opened** at least one finding (#882) — a newly created finding, or a `RESOLVED` one that came back. A finding merely re-seen on a later scan, or an acknowledged finding whose values changed, never notifies, so a persistent finding does not alert every scan. One notification per environment scan, carrying the count | Every active holder of `SCHEMA_CHANGE_MANAGE` (system and custom roles). Org-wide channel fanout; never pages and never opens a ticket — findings carry no severity, so there is nothing to key a paging trigger on. | implemented |
...
- `email/query-executed.html` — `QUERY_EXECUTED` (#627; recurring occurrence outcome — status, rows, duration, SQL preview, and, for successful SELECT occurrences, the results as a `results-<occurrence-id>.csv` attachment rendered by `QueryResultCsvRenderer` from the post-mask `query_request_results` snapshot, capped at 10,000 rows)
...
- `email/schema-change-promotion-submitted.html` — `SCHEMA_CHANGE_PROMOTION_SUBMITTED` (#882; change set, pipeline, environment, promoter)
...
| `SCHEMA_CHANGE_PROMOTION_SUBMITTED` | The target datasource's eligible reviewers (stage-1 plan approvers ∪ datasource reviewer assignments, else REVIEWER ∪ ADMIN role holders), excluding the promoter |
