...
| `governs_apis` | BOOLEAN NOT NULL DEFAULT false (AF-898) — the org opted into API access governance in the first-run wizard. A **visibility hint, not an entitlement**: it adds the "create your first API connector" step to `GET /admin/setup-progress` and, since #926, decides whether the SPA offers the API sidebar sub-sections, the API review-hub tab and the API dashboard widgets. It gates no route, permission or endpoint authorization — `apigov` stays fully usable either way and every deep link keeps working. Settable at `POST /auth/setup`, changeable by an org admin via `PUT /admin/governance-domains` or by a platform admin via `PUT /platform/organizations/{id}`. |
| `governs_deployments` | BOOLEAN NOT NULL DEFAULT false (AF-898) — the same for deployment approval governance: the "create your first deployment pipeline" step, plus the Deployments sidebar sub-sections, the Deployments/Rollbacks review-hub tabs and the three deployment dashboard widgets (#926). |
...
| `principal_type` | ENUM `principal_type`: `HUMAN` \| `SERVICE_ACCOUNT`, NOT NULL DEFAULT `HUMAN` (#868, V173, epic #867) — the person-vs-non-human discriminator. A service account is **still a `users` row** (every actor FK points at `users(id)`; the self-approval invariant is a `UUID.equals` on it) with a 1:1 detail row in [`service_accounts`](#service_accounts-serviceaccounts-868--epic-867). At runtime only `UserAdminService.setPrincipalType`, called from `ServiceAccountProvisioningService.ensureRegistered` in the same transaction as the detail row, ever flips it; the one other writer is the V173 backfill, which flipped exactly the users recorded in `bootstrap_state` as `SERVICE_ACCOUNT`. Enforced on the sign-in surface since #869 — password login, refresh and the SAML / OAuth2 exchanges all reject a `SERVICE_ACCOUNT` (401 `SERVICE_ACCOUNT_SIGN_IN_BLOCKED`); API-key authentication is the only way in. #872 enforces the MCP tool allow-list at every `tools/call` (`serviceaccounts.api.ServiceAccountToolPolicyService`); #873 caps every API-key-authenticated request per identity (`service_accounts.rate_limit_*` when set, the deployment default otherwise — humans' personal keys included). |
...
this row in one transaction) and the `SERVICE_ACCOUNT_MANAGE` permission (`USERS` group, `ADMIN`
only, seeded by `V174`). #871 adds the second caller of the chokepoint — `ServiceAccountAdminService`
behind `/admin/service-accounts` (create / update / deactivate and key issue / rotate / revoke on the
account's behalf, see [04-api-spec.md](04-api-spec.md#service-accounts-adminservice-accounts-service_account_manage-871)) —
and populates `description` / `owner_user_id`; `mcp_tool_allow_list` is enforced at MCP invocation since
#872 and the rate limits by #873. The detail row **is part of the identity**, so unlike other modules' cross-module
references these are real foreign keys: deleting the user deletes the account, deleting the owning
human only detaches ownership. Created by `V173`.
...
| `description` | VARCHAR(500), nullable — free text set from the admin surface (#871) |
| `owner_user_id` | FK → `users` ON DELETE SET NULL, nullable — the human the account acts for (#871). The admin surface only accepts an active `HUMAN` user of the same organization (`422 SERVICE_ACCOUNT_OWNER_INVALID`); it confers nothing — on-behalf-of attribution is #874 |
| `managed_by` | ENUM `service_account_source`: `UI` \| `BOOTSTRAP`, NOT NULL — which surface owns the account's *declared* fields: email, display name and role on `users`, and the `bootstrap_declared` key in `api_keys`. The bootstrap reconciler stamps `BOOTSTRAP` on every create and changed re-run; `POST /admin/service-accounts` stamps `UI` (#871). Note the reconciler only *uses* display name and role when it creates the user — it never re-asserts them on an existing one — so what keeps a `BOOTSTRAP` account's declared fields in sync with the YAML is the admin surface refusing to change them (`409 SERVICE_ACCOUNT_BOOTSTRAP_MANAGED`; sending the current value is a no-op). `active` is not declared: the reconciler never resurrects a deactivated account, and `PUT { "active": true }` is the remediation. The remaining columns are UI-owned on both and are never touched by a bootstrap re-run |
| `mcp_tool_allow_list` | TEXT[], nullable — MCP tool names the account may invoke (set via the admin surface since #871, validated against the catalog — `422 SERVICE_ACCOUNT_UNKNOWN_MCP_TOOL`; enforced at every MCP `tools/call` since #872 by `ServiceAccountToolPolicyService` behind the `mcp` module's `GuardedToolCallback` — `tools/list` is not filtered). **`NULL` = every tool, `'{}'` = none.** Tool names rather than an enum type, so a renamed tool never makes a row unreadable; the catalog is `serviceaccounts.api.McpToolName`, kept equal to the registered `@Tool` set by `McpToolNameParityTest` |
| `rate_limit_per_minute` | INTEGER, nullable — set via the admin surface (#871, `@Positive`), enforced on every API-key request by `DefaultServiceAccountRateLimiter` (#873); `NULL` = the deployment default `accessflow.serviceaccounts.rate-limit.requests-per-minute` (120) |
| `rate_limit_per_day` | INTEGER, nullable — set via the admin surface (#871, `@Positive`), enforced by #873; `NULL` = the deployment default `accessflow.serviceaccounts.rate-limit.requests-per-day` (0 = unlimited) |
...
| `db_type` | ENUM: `POSTGRESQL` \| `MYSQL` \| `MARIADB` \| `ORACLE` \| `MSSQL` \| `CUSTOM` \| `MONGODB` \| `COUCHBASE` \| `REDIS` \| `CASSANDRA` \| `SCYLLADB` \| `ELASTICSEARCH` \| `OPENSEARCH` \| `DYNAMODB` \| `NEO4J` \| `SNOWFLAKE` \| `BIGQUERY` \| `DATABRICKS` — `CUSTOM` is paired with either `connector_id` (a catalog connector such as ClickHouse) or `custom_driver_id` + `jdbc_url_override` (an uploaded driver). `MONGODB` is the NoSQL document engine: a native (non-JDBC) connector whose engine plugin is resolved on demand through the connector catalog (AF-414), using the standard `host`/`port`/`database_name`/`username`/`password`/`ssl_mode` fields (no `connector_id`, no `jdbc_url_override`). `COUCHBASE` is the second NoSQL document engine (AF-412, migration `V72`): the SQL++ engine plugin, same on-demand model, with `database_name` holding the bucket. `REDIS` is the NoSQL **key-value** engine (AF-419, migration `V74`, category `KEY_VALUE`): the native Jedis engine plugin, same on-demand model, with `database_name` holding the numeric DB index (default `0`). `CASSANDRA` is the NoSQL **wide-column** engine (AF-421, migration `V75`, category `WIDE_COLUMN`): the native DataStax-driver CQL engine plugin, same on-demand model, with `database_name` holding the keyspace and the `local_datacenter` column required. `SCYLLADB` (migration `V76`) is CQL-compatible and served by the very same Cassandra plugin JAR (which registers a second `QueryEngine` provider with `engineId="scylladb"`); it has its own `db_type` only because the catalog allows one connector per non-`CUSTOM` dialect. `ELASTICSEARCH` is the NoSQL **search** engine (AF-420, migration `V78`, category `SEARCH`): the native low-level-REST-client engine plugin, same on-demand model, queried with a JSON envelope; `database_name` is optional (it only scopes introspection — the index is named in the query) and the optional `api_key_encrypted` column carries an API key as an alternative to `username`/`password`. `OPENSEARCH` (migration `V79`) is wire-compatible and served by the very same Elasticsearch plugin JAR (which registers a second `QueryEngine` provider with `engineId="opensearch"`); it has its own `db_type` for the same one-connector-per-dialect reason. `DYNAMODB` is the NoSQL **key-value** engine (AF-422, migration `V81`, category `KEY_VALUE`): the native AWS-SDK-v2 PartiQL engine plugin, same on-demand model, but its connection is **cloud credentials + region, not host/port** — `database_name` holds the AWS region, `username` the access key id, `password_encrypted` the secret access key, and `jdbc_url_override` an optional custom endpoint (DynamoDB Local / VPC); `host`/`port` are unused. `NEO4J` is the NoSQL **graph** engine (AF-423, migration `V82`, category `GRAPH`): the native Neo4j-Java-driver Cypher engine plugin over Bolt, same on-demand model, with `database_name` holding the Neo4j database (required) and the SSL mode encoded in the Bolt scheme; like DynamoDB it also accepts a full `bolt://` / `neo4j+s://` URI in `jdbc_url_override` (Aura / clustered routing) in place of host/port. `SNOWFLAKE`, `BIGQUERY`, and `DATABRICKS` are the cloud **data-warehouse** engines (AF-629, migrations `V124`/`V125`/`V126`, category `WAREHOUSE` — SQL-dialect but engine-managed, since their auth models don't fit pooled JDBC): `SNOWFLAKE` uses `host` (account host), `database_name` (database), `username`, and `password_encrypted` holding a password **or** a PKCS#8 private-key PEM (key-pair JWT; a passphrase-protected PEM additionally needs `private_key_passphrase_encrypted`), with `jdbc_url_override` an optional full `jdbc:snowflake://` URL carrying warehouse/role/schema parameters (`port` unused); `BIGQUERY` is cloud-credentials only — `database_name` holds the GCP project (optionally `project.dataset` to pin a default dataset), `password_encrypted` the service-account key JSON, and `jdbc_url_override` an optional custom endpoint (emulator) — `host`/`port`/`username` unused; `DATABRICKS` uses `host` (workspace host), `password_encrypted` (personal access token), a **required** `jdbc_url_override` holding the SQL-warehouse HTTP path (`/sql/1.0/warehouses/<id>`), and an optional `database_name` (Unity Catalog catalog). See [14-connectors.md](./14-connectors.md), [05-backend.md → MongoDB engine](./05-backend.md#mongodb-engine), [05-backend.md → Couchbase engine](./05-backend.md#couchbase-engine), [05-backend.md → Redis engine](./05-backend.md#redis-engine), [05-backend.md → Cassandra engine](./05-backend.md#cassandra-engine), [05-backend.md → Elasticsearch engine](./05-backend.md#elasticsearch-engine), [05-backend.md → DynamoDB engine](./05-backend.md#dynamodb-engine), [05-backend.md → Neo4j engine](./05-backend.md#neo4j-engine), [05-backend.md → Snowflake engine](./05-backend.md#snowflake-engine), [05-backend.md → BigQuery engine](./05-backend.md#bigquery-engine), and [05-backend.md → Databricks engine](./05-backend.md#databricks-engine). Added by migration `V71`. |
...
| `max_bytes_scanned_per_query` | BIGINT nullable, `CHECK > 0` (#941, V192) — the **bytes-scanned cap**: the largest pre-flight scan estimate a query on this datasource may have, in raw bytes. NULL = no cap. Configurable only on the engines whose dry-run reports a bytes estimate — BigQuery, Snowflake, Databricks (`core.api.BytesScannedCapSupport`); any other engine is refused with 422 `BYTES_SCANNED_CAP_NOT_SUPPORTED`. Set on create; on update `max_bytes_scanned_per_query` (null = unchanged) / `clear_max_bytes_scanned_per_query` (remove). Enforced when the query leaves `PENDING_AI` and again just before execution — see [05-backend.md → Bytes-scanned cost caps](05-backend.md#bytes-scanned-cost-caps-941) |
| `bytes_cap_missing_estimate` | ENUM `bytes_cap_missing_estimate_action` NOT NULL DEFAULT `REQUIRE_REVIEW` (#941, V192) — what the cap does when a query has **no** bytes estimate (the estimate failed, DDL, a shape the engine cannot plan): `REQUIRE_REVIEW` suppresses every automatic approval so a person decides; `REJECT` refuses the query. Inert while no cap applies |
| `is_active` | BOOLEAN DEFAULT true |
| `created_at` | TIMESTAMPTZ |
...
| `bytes_scanned_limit_override` | BIGINT nullable, `CHECK > 0` (#941, V192) — a bytes-scanned cap for this grantee. Combined with the datasource's `max_bytes_scanned_per_query`, the **smallest** applies; across a user's direct and group grants the smallest non-null value wins, like `row_limit_override`. A JIT approval that replaces a time-boxed row carries the replaced row's value over, so an approval never widens it. Refused with 422 `BYTES_SCANNED_CAP_NOT_SUPPORTED` on an engine without a bytes estimate |
...
`row_limit_override` and `bytes_scanned_limit_override` (#941) merge to the **smallest** non-null
value so a wide group grant can never raise a tight per-user cap (#933), and the deny-lists — `denied_schemas` / `denied_tables` (#939),
...
| `bytes_scanned_limit_override` | BIGINT nullable (#941, V192) — same semantics as the per-user table; merged most-restrictive |
...
names no schema, or the policy names no schema. A database or project prefix in front of the schema
(`mydb.crm.customer`) still matches. So `crm.customer` also matches an unqualified `customer` in the
SQL: neither leaving the schema off nor adding a database name loosens the cap. References are
compared case-insensitively and suffix-wise, never by splitting them on dots, so a policy whose table
name contains dots (an Elasticsearch index, a BigQuery table) is compared as one name.
...
matches no policy and falls back to the datasource cap. Only SELECT result sets are capped;
the ids of the lowest-cap matching policies ride on `QUERY_EXECUTED` (and
`QUERY_BREAK_GLASS_EXECUTED`) metadata as `applied_row_limit_policy_ids` — recorded only when that
cap is at or below both the grant override and the datasource's `max_rows_per_query`, so a policy
that something tighter undercut is not named (the global ceiling is not compared). Grouped
executions enforce the cap but, like masking and row-security ids, write only the group-level audit
row. The table preview (`GET /datasources/{id}/sample-rows`) is capped the same way.
...
| `recurrence_next_run_at` | TIMESTAMPTZ nullable (`V132`) — the scheduler cursor: next occurrence instant, advanced atomically with each occurrence-row insert via a locked compare-and-set on the observed due value (racing callers fire each due window exactly once). Cleared (`NULL`) on series completion, fail-closed halt, or cancellation. Partial index `idx_query_requests_recurrence_due ON query_requests(recurrence_next_run_at) WHERE recurrence_next_run_at IS NOT NULL` backs the `RecurringQueryRunJob` poll. |
...
PENDING_AI → PENDING_REVIEW → APPROVED → EXECUTED
           ...
           ↘ REJECTED       (bytes-scanned cap, #941 — estimate over the cap, or none under
...
| `estimated_bytes_scanned` | BIGINT nullable (#941, V192) — the warehouse engine's native pre-flight scan estimate in raw bytes (AF-634: BigQuery dry-run `totalBytesProcessed`, Snowflake `bytesAssigned`, Databricks `sizeInBytes`); NULL for every other engine. Read by the `estimated_bytes_scanned` routing condition and the bytes-scanned cap |
...
| `AUDIT_LOG_EXPORTED` | Admin called `GET /admin/audit-log/export.csv`. Resource: `audit_log`, no resource id. Metadata captures the export filter (`action`, `resource_type`, `actor_id`, `resource_id`, `from`, `to`, `filter_on_behalf_of_user_id` — never the `on_behalf_of_user_id` attribution key, #875 — and `filter_application_name`, never the `application_name` key, which names the application that made the export request itself, #938) and the row counts (`matched_rows`, `truncated`). |
...
`requestgroups` group (`request_group_id`) whose status is projected back onto `status` by
`SchemaChangePromotionStatusListener` — `PENDING_REVIEW → IN_REVIEW`, `APPROVED → APPROVED`,
`EXECUTED → APPLIED`, `PARTIALLY_EXECUTED → PARTIALLY_APPLIED`, `FAILED → FAILED`,
`REJECTED`/`TIMED_OUT`/`CANCELLED → CANCELLED` — and only ever forwards, because those events arrive asynchronously and can reorder.
`statements_checksum` is recomputed from the statement rows at submission as evidence of exactly
what was sent; `schema_snapshot` is the post-apply introspection that the `PROMOTION_SNAPSHOT`
drift baseline reads, written just after the `APPLIED` transition commits by a second listener (a
failed or still-running introspection leaves it null and never delays the transition). Only `APPLIED` counts toward the ladder gate (#880).
