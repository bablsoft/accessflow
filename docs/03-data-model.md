...
| `ACCESS_SIMULATION_RUN` | An admin traced a hypothetical request through the live evaluators (`POST /admin/access-simulations`) or read who can reach a table (`GET /admin/effective-access`) (AF-859). Both are read-only. Resource: `datasource`, resource id = the datasource asked about. Metadata differs by endpoint — the trace records `simulated_user_id`, `ai_outcome`, `step_count` and optional `risk_level` / `resulting_status`; the reverse index records `table`, `capability` and `row_count` (total matching users, not the page size). **Neither carries the SQL**: `DATASOURCE_PERMISSION_MANAGE` does not otherwise grant read access to query text. |
