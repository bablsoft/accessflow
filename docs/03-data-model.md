...
| `AUDIT_LOG_EXPORTED` | Admin called `GET /admin/audit-log/export.csv`. Resource: `audit_log`, no resource id. Metadata captures the export filter (`action`, `resource_type`, `actor_id`, `resource_id`, `from`, `to`, `filter_on_behalf_of_user_id` — never the `on_behalf_of_user_id` attribution key, #875) and the row counts (`matched_rows`, `truncated`). |
