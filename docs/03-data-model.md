...
| `SCHEMA_CHANGE_PROMOTION_APPLIED` / `_PARTIALLY_APPLIED` / `_FAILED` | The promotion's request group finished (#880). Resource: `schema_change_promotion`. **Null actor** — projected from the group by an event listener. Metadata: `trigger: "request_group"`, `request_group_id`, `group_status`, `change_set_id`, `environment_id`, plus `error_message` on the two failure outcomes. `_PARTIALLY_APPLIED` means earlier statements landed and a later one failed: there is no rollback. |
| `SCHEMA_CHANGE_PROMOTION_CANCELLED` | A promotion was cancelled before its group was approved (#880). Resource: `schema_change_promotion`. Actor = the caller; metadata `request_group_id` and `cancelled_on_behalf_of_submitter: true`, because the group's own cancel is submitter-only and is therefore invoked as the promoter. |
...
`SchemaChangePromotionStatusListener` — `PENDING_REVIEW → IN_REVIEW`, `EXECUTED → APPLIED`,
`PARTIALLY_EXECUTED → PARTIALLY_APPLIED`, `REJECTED`/`TIMED_OUT`/`CANCELLED → CANCELLED` — and
only ever forwards, because those events arrive asynchronously and can reorder.
...
drift baseline reads, written on the transition to `APPLIED` (a failed introspection leaves it
null and keeps the transition). Only `APPLIED` counts toward the ladder gate (#880).
