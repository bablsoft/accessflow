...
and populates `description` / `owner_user_id`; `mcp_tool_allow_list` is enforced by #872 and the rate
limits by #873. The detail row **is part of the identity**, so unlike other modules' cross-module
...
| `mcp_tool_allow_list` | TEXT[], nullable — MCP tool names the account may invoke (set via the admin surface since #871, validated against the catalog — `422 SERVICE_ACCOUNT_UNKNOWN_MCP_TOOL`; *enforced by #872*). **`NULL` = every tool, `'{}'` = none.** Tool names rather than an enum type, so a renamed tool never makes a row unreadable; the catalog is `serviceaccounts.api.McpToolName`, kept equal to the registered `@Tool` set by `McpToolNameParityTest` |
