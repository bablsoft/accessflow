...
`VALIDATION_ERROR` (`error.datasource_body_unreadable`), too many is 400 (`validation.denied_shapes.too_many`).
...
| `SQL_PARSE` | `query_type`, `referenced_tables`, `transactional`, `has_where_clause`, `has_limit_clause`, `query_shapes` (#940 — the statement's shapes in declaration order), `shapes_analyzed` (`false` when the shape could not be read — a non-SQL engine) | whenever the statement parsed; `{}` when it did not |
