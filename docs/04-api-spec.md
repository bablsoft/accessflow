...
> **`risk_level` without `risk_score`.** Routing conditions on the numeric score require
> `risk_score >= 0`, but the grant-covered fast path (#582) gates on the **level alone**. Supplying
> `{"risk_level": "HIGH"}` with no score therefore suppresses the fast path *and* silently disables every
> score-based condition. Send both, or neither.
  ...
  "steps": [
    ...
    },
    {
      ...
      "details": {
        ...
        "policies": [
          { "policy_id": "a0c1…", "name": "Block payroll deletes", "priority": 5, "action": "AUTO_REJECT", "matched": false },
          { "policy_id": "b1f0…", "name": "Escalate prod payment writes", "priority": 10, "action": "ESCALATE", "required_approvals": 2, "matched": true },
          { "policy_id": "c2e3…", "name": "Auto-approve low-risk reads", "priority": 20, "action": "AUTO_APPROVE", "matched": false }
        ],
    ...
    { "step": "REVIEW_PLAN", "outcome": "SKIP", "reason": "A routing policy already decided this request", "details": { "requires_human_approval": true, "auto_approve_reads": false, "min_approvals_required": 1 } },
    {
      ...
      "outcome": "ALLOW",
      "reason": "2 reviewers could act on this request",
      "details": { "reviewers": [ { "user_id": "4d2b…", "email": "dana@example.com" } ], "submitter_excluded": true }
    },
    {
      "step": "ROW_SECURITY",
      "outcome": "MATCH",
      "reason": "Row-security predicates would be injected",
      "details": { "engine_id": "jdbc", "row_security_outcome": "APPLIED", "applied_policy_ids": ["9f31…"] }
    },
    {
      "step": "MASKING",
      "outcome": "MATCH",
      "reason": "1 masking policy resolves for the referenced tables",
      "details": { "policies": [ { "policy_id": "6cab…", "column_ref": "payments.card_number", "mask_type": "PARTIAL" } ] }
    },
...
is a step-specific object whose keys are documented above and are stable per step.
...
rather than implying a precision it does not have. In addition to the shared
[policy-simulator caveats](#policy-simulator-af-630):
...
that grants it. `sources[].kind` is one of:
...
| `JIT_GRANT` | A time-boxed direct row correlated to an active `APPROVED` JIT grant. `pre_approve_queries: true` additionally means queries under it skip review (#582) |
...
`ALLOW_LISTED` carries the entry that actually covered the query in `covering_allow_list_entry` —
the schema (`"public"`) or the qualified table (`"public.payments"`).
...
**Audit.** Every call writes one `ACCESS_SIMULATION_RUN` audit row against the datasource, carrying the
table and capability queried and the number of rows returned.
...
Validation: `name` non-blank ≤ 255; `provider` ∈ {OPENAI, ANTHROPIC, OLLAMA, OPENAI_COMPATIBLE, HUGGING_FACE} — `VOYAGE` is an embeddings-only provider and returns **400** `AI_CONFIG_PROVIDER_INVALID` here (AF-918); `model` non-blank ≤ 100; `endpoint` ≤ 500 (**required** when `provider = OPENAI_COMPATIBLE`); `api_key` ≤ 4096 (optional for OLLAMA, OPENAI_COMPATIBLE, and HUGGING_FACE); `timeout_ms` ∈ [1000, 600000]; `max_prompt_tokens` and `max_completion_tokens` ∈ [100, 200000]; `system_prompt_template` ≤ 20000 and — when non-blank — **must contain the `{{sql}}` placeholder** (else **400** `AI_CONFIG_INVALID_PROMPT`). Omit it (or send blank) to use the built-in default; fetch the default via `GET /admin/ai-configs/prompt-default`. `langfuse_prompt_name` and `langfuse_prompt_label` ≤ 255 each (both optional).

**RAG fields (AF-336, all optional):** `rag_enabled` (boolean), `rag_store_type` (`PGVECTOR` | `QDRANT`), `rag_top_k` ∈ [1, 20], `rag_similarity_threshold` ∈ [0, 1], `rag_endpoint` ≤ 500, `rag_collection` ≤ 255, `rag_api_key` ≤ 4096, `embedding_provider` (`ai_provider`, **not** `ANTHROPIC`; `VOYAGE` is valid here and nowhere else), `embedding_model` ≤ 100, `embedding_endpoint` ≤ 500, `embedding_api_key` ≤ 4096, `embedding_dimensions` ∈ [0, 4096]. When `rag_enabled = true` the resulting row must have a `rag_store_type`, an `embedding_provider` (≠ `ANTHROPIC`) and an `embedding_model`; `QDRANT` additionally requires `rag_endpoint` + `rag_collection`. A violation returns **400** `RAG_CONFIG_INVALID`.

`embedding_dimensions` (AF-918) requests a vector length; omit it — or send `0`, which means "no override" on create and "clear the stored one" on update — to use the provider's default. Three rules, each a **400** `RAG_CONFIG_INVALID`:

- The provider must be able to honour a requested length at all. `OPENAI` / `OPENAI_COMPATIBLE` / `HUGGING_FACE` send it as OpenAI's `dimensions` parameter and `VOYAGE` as `output_dimension`; `OLLAMA` returns whatever its model natively emits and so rejects the field outright, because accepting it would let a row satisfy the width check below on paper and still fail at ingest.
- It must be a length the provider can emit. `VOYAGE` allows only **256 / 512 / 1024 / 2048**; the others accept any positive value up to 4096.
- On the `PGVECTOR` store it must match the deployment's `ACCESSFLOW_RAG_PGVECTOR_DIMENSIONS`, which is frozen into the `vector(N)` column at the first migration. Both widths appear in the `detail`. A `VOYAGE` row is checked even when the field is blank, against Voyage's pinned 1024 default — so on a default (1536) deployment Voyage needs an external `QDRANT` store, or a database provisioned at one of Voyage's widths before its first migration. `rag_api_key` / `embedding_api_key` follow the same masking semantics as `api_key`; the keys are never returned (a stored key surfaces as `********`).

**Fallback pool field (AF-458, optional):** `fallback_priority` (integer 0–100) marks the configuration as an organization-wide fallback — when the configuration handling a request fails, fallbacks are retried once each in ascending priority order (lower = first). On **create**, `null`/absent (or any negative value) means "not a fallback". On **update**, `null`/absent leaves the stored value unchanged and `-1` clears it (values outside `-1..100` return **400**). The response carries `fallback_priority` (`null` when the configuration is not a fallback).

**Orchestration + guardrail fields (AF-450, all optional):** `orchestration_enabled` (boolean), `voting_strategy` (`WEIGHTED_AVERAGE` | `MAX_RISK` | `MAJORITY`), `voting_weight` (> 0, the primary model's weight), `guardrail_patterns` (array of ≤ 50 case-insensitive regex strings, each ≤ 500 chars), and `models` (≤ 20 orchestration members). Each member is `{ id?, provider, model, endpoint?, api_key?, weight, enabled }`: `provider` required, `model` ≤ 100 required, `endpoint` ≤ 500 (**required** for an `OPENAI_COMPATIBLE` member), `api_key` ≤ 4096 (same masking as the primary — `********` keeps a member's stored key), `weight` > 0. On update, `models` **replaces** the member set (members carrying a known `id` are updated in place, preserving a masked key; members without a matching `id` are inserted; existing members absent from the list are deleted). A guardrail pattern that is not a valid regex, a non-positive weight, a member missing its provider/model, a `VOYAGE` member (embeddings only — it cannot vote), or an `OPENAI_COMPATIBLE` member with no endpoint returns **400** `AI_CONFIG_ORCHESTRATION_INVALID`. When orchestration is enabled, the bound configuration runs the primary model plus the enabled members in parallel and aggregates per `voting_strategy`. Guardrail patterns are matched against the submitted SQL (analysis) and NL prompt (text-to-SQL) before any model call — a match returns **422** `AI_GUARDRAIL_BLOCKED` on the preview / text-to-SQL endpoints and records a sentinel `CRITICAL` analysis on the async submitted-query path.
...
**Response 400:** Validation error; `provider = OPENAI_COMPATIBLE` with a blank `endpoint` (`error: AI_CONFIG_ENDPOINT_REQUIRED`); `provider = VOYAGE`, or a `VOYAGE` orchestration member, because Voyage has no chat API (`error: AI_CONFIG_PROVIDER_INVALID` / `AI_CONFIG_ORCHESTRATION_INVALID`); or a custom `system_prompt_template` missing the `{{sql}}` placeholder (`error: AI_CONFIG_INVALID_PROMPT`).
...
**Response 400:** Resulting `provider = OPENAI_COMPATIBLE` with a blank `endpoint` (`error: AI_CONFIG_ENDPOINT_REQUIRED`); a resulting `provider = VOYAGE`, or a `VOYAGE` orchestration member, because Voyage has no chat API (`error: AI_CONFIG_PROVIDER_INVALID` / `AI_CONFIG_ORCHESTRATION_INVALID`); inconsistent RAG settings, including an unusable `embedding_dimensions` (`error: RAG_CONFIG_INVALID`); or a custom `system_prompt_template` missing the `{{sql}}` placeholder (`error: AI_CONFIG_INVALID_PROMPT`).
...
- `ai_provider_configured`: at least one `ai_config` row for the organization is usable — its provider can drive a chat model, and either has a stored API key **or** may run keyless (`OLLAMA`, `OPENAI_COMPATIBLE`, `HUGGING_FACE`, all self-hosted endpoints). An embedding-only provider never counts (AF-918).
...
| `query.prediction_complete` | The advisory approval-outcome prediction finished (AF-645) — pushed to the review plan's **first-stage** approvers only (the same set `review.new_request` targets), so the review queue and detail view refetch. The submitter is deliberately excluded: `GET /queries/{id}` withholds the block from them, so a refetch trigger would fetch nothing. `probability` is `null` on the skipped / failed sentinel rows | `query_id`, `probability` |
