...
- **Nested document values (AF-658).** `NestedValueFlattener` walks `Map`/`List` cell values —
  the shape MongoDB, Couchbase, Elasticsearch, DynamoDB, Neo4j and BigQuery put inside a cell,
  since their columns are the ordered union of *top-level* fields — and flattens the leaves to
  **dot-path pseudo-columns** (`profile.contact.email`). Map keys consume a path segment;
  **lists are transparent fan-out points and never contribute an index segment**, so
  ...
  Couchbase, Elasticsearch, DynamoDB and BigQuery result mappers use for masking, so confirming the
  finding creates an AF-447 tag that redacts the very leaf that was detected. (Neo4j resolves masks
  by `Label.property` instead, so a nested finding there is still worth confirming as a
  classification, but its derived masking follows Neo4j's own bare-property rule.) Only non-blank
  **string** leaves are collected, exactly as for top-level cells. Four bounds keep an arbitrarily shaped document from turning a bounded sample
  into an unbounded scan: `max-nested-depth` (default 5), a per-row visit allowance
  `max-nested-leaves-per-row` (default 100, charged per node touched — top-level scalar cells are
  free, so wide relational tables are unaffected), a 256-character path-name limit, and a
  500-distinct-path ceiling per table (beyond it known paths keep filling, no new ones open; logged
  at INFO, and deliberately **not** flagged `partial`, which means "tables were skipped"). Because
