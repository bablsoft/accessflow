...
- **Cache:** a Caffeine `Cache<(organizationId, datasourceId), snapshot>` with `expireAfterWrite` = `accessflow.proxy.health.cache-ttl` (default `PT30S`, env `ACCESSFLOW_PROXY_HEALTH_CACHE_TTL`). The page is resolved with `cache.getAll(keys, …)` so a single batched stats query fills all cache-miss rows at once (no N+1) while keeping the per-`(org, datasource)` TTL and tenant isolation — the org is part of the key, so snapshots are never cross-served between tenants.
