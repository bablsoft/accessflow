...
`db_type=MONGODB` datasources are a NoSQL document engine — not JDBC. Everything MongoDB-specific lives under `proxy/internal/mongo/` (plus connection-test / introspection under `core/internal/mongo/`); the rest of the platform (submission, AI analysis, review workflow, audit, permissions, result storage, the `/queries/{id}/results` endpoint) is unchanged. Two clean dispatch points by `DbType` keep the SQL path untouched:

- **Validation** — the workflow layer calls `proxy.api.QueryParser.parse(query, dbType)` (impl `DefaultQueryParser`) instead of `SqlParserService` directly. For `MONGODB` it routes to `MongoQueryParser`; otherwise to the JSqlParser-backed `SqlParserService`. Both return the engine-neutral `SqlParseResult` (`referencedTables` carries collection names).
- **Execution** — `DefaultQueryExecutor.execute(...)` branches early: `descriptor.dbType() == MONGODB` delegates to `MongoQueryExecutor`.

**Query forms.** `MongoQueryParser` parses **both** supported forms into an internal `MongoCommand`, auto-detecting by leading token:
- **Shell** (leading `db.`): `db.users.find({ age: { $gt: 21 } }).limit(10).sort({ name: 1 })`, `db.orders.aggregate([…])`, `db.users.insertOne({…})`, `updateMany`/`replaceOne`/`findOneAndUpdate`, `deleteOne`/`deleteMany`, `db.users.createIndex({…})`, `db.createCollection('x')`, `db.x.drop()`, `db.x.distinct('field', {…})`, `db.x.countDocuments({…})`, and the `db.getCollection("name")…` accessor. Relaxed JSON (single quotes, unquoted keys, comments) is accepted via a Jackson reader.
...
**Connection management.** `MongoClientManager` caches one native `MongoClient` per datasource (the driver pools internally), evicted on `DatasourceConfigChangedEvent`/`DatasourceDeactivatedEvent` by `MongoClientEvictionListener` — the document-engine analogue of `DefaultDatasourceConnectionPoolManager` + `DatasourcePoolEvictionListener`. The connection-string URI is built by the shared `core.api.MongoConnectionStringFactory` (also used by the admin connection-test / introspection path) from host/port/database/credentials/SSL; tuning via `accessflow.proxy.mongo.*` (`connect-timeout`, `server-selection-timeout`, `max-pool-size`). A datasource with a configured read replica routes `SELECT`s with `ReadPreference.secondaryPreferred()`.
...
**Connection test + introspection** (`core/internal/mongo/`): `MongoConnectionProbe` runs a `ping`; `MongoSchemaIntrospector` lists collections and samples documents to infer fields, returning the same `DatabaseSchemaView` (schema = database, tables = collections, columns = fields, `_id` flagged primary key) used by the ER diagram, editor autocomplete, and AI schema context.
...
### Text-to-SQL generation (AF-335)

Per-datasource `text_to_sql_enabled` lets a user draft SQL from a natural-language prompt. The capability **reuses the same `AiAnalyzerStrategy` infrastructure** as risk analysis — the strategy interface gained a second method, `generateSql(prompt, dbType, schemaContext, language, aiConfigId)`, implemented by every provider adapter (Anthropic / OpenAI-compatible / Ollama) and routed through the same `AiAnalyzerStrategyHolder` per-config delegate cache and eviction listeners. A shared `ChatModelInvoker` performs the Spring AI `ChatModel` call and token/model extraction so each adapter's `generateSql` stays a thin wrapper.

- `TextToSqlService.generateSql(datasourceId, prompt, userId, organizationId, isAdmin)` (impl `DefaultTextToSqlService`) mirrors `analyzePreview`: it resolves the datasource, **requires `text_to_sql_enabled`** (`TextToSqlDisabledException` → HTTP 409), **requires a bound `ai_config`** (`TextToSqlNotConfiguredException` → HTTP 400), verifies the config belongs to the org, introspects the schema (honouring the caller's restricted columns), resolves the org's review language, then calls the strategy. **No persistence, no events, no query request is created.**
- The generation prompt is a fixed default template (`SystemPromptRenderer.DEFAULT_SQL_GENERATION_TEMPLATE`, tokens `{{db_type}}`, `{{schema_context}}`, `{{language}}`, `{{user_request}}`). It steers the model toward a single dialect-appropriate, schema-grounded statement, instructs it to **never reference `*RESTRICTED*` columns**, and to return a strict JSON envelope `{"sql": "..."}` parsed by `SqlGenerationResponseParser` (malformed output → `AiAnalysisParseException` → HTTP 422; provider failure → `AiAnalysisException` → HTTP 503). Per-config custom prompt / Langfuse override for generation is a deliberate follow-up — for v1 the default template is used directly and the generation path is not traced to Langfuse.
- **Governance is preserved end to end:** the returned SQL is only a draft that lands in the editor. The user still submits it through `POST /api/v1/queries`, where JSqlParser validation, the schema allow-list, permission checks, AI risk analysis, and human review all run as normal — so text-to-SQL can never bypass the approval pipeline or column masking. Exposed at `POST /api/v1/queries/generate-sql` (`TextToSqlController`); handlers live in the existing `AiAnalysisExceptionHandler`.
