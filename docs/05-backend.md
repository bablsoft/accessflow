...
- `DefaultQueryExecutor` — `@Service`. Resolves the datasource descriptor, computes `effectiveMaxRows = min(override ?? datasource.maxRowsPerQuery, datasource.maxRowsPerQuery, accessflow.proxy.execution.max-rows)` — an override only ever lowers the cap (#933). `DefaultQueryLifecycleService.doExecute` (direct, scheduled, recurring and break-glass runs) and `GroupExecutionService` (grouped members) pass the submitter's merged `row_limit_override` as the override, read from the same `DatasourceUserPermissionLookupService.findFor` call that supplies `restrictedColumns`; the merge takes the smallest non-null override across the direct and group grants and `effectiveTimeout = override ?? accessflow.proxy.execution.statement-timeout`, then branches on the request's `transactional` flag:
