...
- `DefaultQueryExecutor` — `@Service`. Resolves the datasource descriptor, computes `effectiveMaxRows = min(override ?? datasource.maxRowsPerQuery, datasource.maxRowsPerQuery, accessflow.proxy.execution.max-rows)` (an override only ever lowers the cap — #933) and `effectiveTimeout = override ?? accessflow.proxy.execution.statement-timeout`, then branches on the request's `transactional` flag (below). The row override comes from the submitter's effective `row_limit_override`: `DefaultQueryLifecycleService.doExecute` (direct, scheduled, recurring and break-glass runs) and `GroupExecutionService` (grouped members) read it from the same `DatasourceUserPermissionLookupService.findFor` call that supplies `restrictedColumns`, where the merge takes the smallest non-null override across the direct and group grants. Both callers then lower it further by the per-table row-limit policies (#934): `core.api.RowLimitPolicyResolutionService.resolve(org, datasource, submitter, parsed.referencedTables())` returns the lowest `max_rows` among the enabled policies that apply to the submitter and name a referenced table (lenient, case-insensitive suffix matching — an unqualified reference or a schema-less policy matches the table in any schema, and a database-prefixed `db.schema.table` still matches a `schema.table` policy; an empty `referencedTables` matches nothing), folded in as `min(grantOverride, policyCap)`. The executor itself is unchanged: `clampMaxRows` still clamps to the datasource and global caps, so a policy can only lower the limit. `doExecute` records the lowest-cap policy ids as `applied_row_limit_policy_ids` on the success audit row, for SELECTs only and only when that cap is at or below both the grant override and the descriptor's `maxRowsPerQuery`; `GroupExecutionService` enforces but does not audit per member.
  - Non-transactional (default):
    ...
    ```
    `autoCommit` is left at the HikariCP default (`true`). On PostgreSQL the pool's `readOnlyMode=always` driver property is what makes `setReadOnly(true)` effective under autocommit. The `+1` row beyond the cap is read solely to mark the result `truncated=true` and is then discarded.
...
- A statement that writes from inside a query shape — a data-modifying `WITH` item at any depth, `SELECT … INTO` a table / temp table, MySQL `INTO OUTFILE` / `INTO DUMPFILE` — is rejected with 422 `error.sql_embedded_write_not_allowed`, never classified `SELECT`. See [docs/07-security.md → SQL Injection Prevention](07-security.md#sql-injection-prevention).
- PostgreSQL pools set pgjdbc `readOnlyMode=always`, so `setReadOnly(true)` on the `SELECT` path is enforced under autocommit (pgjdbc's default `transaction` mode ignores it). Not guaranteed behind a transaction-mode pooler.
...
`SqlParserService` populates `SqlParseResult.referencedTables` by walking each parsed `Statement` with `SqlStatementInspector`, a `TablesNamesFinder` subclass that records tables itself and flags the embedded writes above in the same pass. The finder's own result is not trusted: it removes every name matching *any* derived-table, LATERAL or `WITH` alias in the statement, so an alias named like a real table erased that table (an incomplete set fails open). The inspector never lets a derived-table alias hide a table, and lets a `WITH` name hide an unqualified table only inside its declaring statement and only after its own body (all names of a `WITH RECURSIVE` list are visible throughout the list); a `WITH` item whose declaring statement it does not recognise hides nothing. It also descends into positions the finder skips — window `PARTITION BY`, a function call's aggregate `ORDER BY` / `LIMIT` / `HAVING` / keyword / named arguments, array subscripts, `TOP`, and `XMLTABLE` / `JSON_TABLE` in `FROM` (the finder's `TableFunction` visit loses their arguments). `RowSecurityRewriter` uses the same walker. Functions that execute SQL text (`query_to_xml`, `dblink`, `OPENQUERY`) remain opaque — see docs/07-security.md. If the walk throws on a `SELECT` / `INSERT` / `UPDATE` / `DELETE`, the statement is rejected with 422 `error.sql_analysis_failed`; only DDL / `OTHER` (gated by `can_ddl` / write access, not the allow-list) fall back to an empty set. Single-statement input yields the table set for that statement; a `BEGIN; …; COMMIT;` envelope yields the union across every inner statement. JSqlParser returns fully-qualified names via `Table.getFullyQualifiedName()` — `schema.table` when the writer qualified it, `table` when they didn't. The parser strips ASCII identifier quotes (`"`, `` ` ``, `[`, `]`) and ASCII-lowercases the result so admin-typed allow-list entries match user SQL regardless of quoting style.
...
- **Data source = immutable snapshots.** Both reports run over `query_snapshots` (AF-449), not live
  ...
  truncation. Regulatory-audit-trail rows carry the snapshot's `effective_sql` (#937) as well as
  `sql_text`; the CSV writer emits it as an `effective_sql` column and the PDF writer as an
  **Effective SQL** column. `DefaultComplianceReportService` (`compliance.internal`) validates the period
...
- **Job names.** A job is named after its class (every AccessFlow job is one `@Scheduled` method
  per class); a class declaring several (a third-party scheduler bean, say) is listed per method as
