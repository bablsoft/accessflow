...
- PostgreSQL pools set pgjdbc `readOnlyMode=always`, so `setReadOnly(true)` on the `SELECT` path is enforced under autocommit (pgjdbc's default `transaction` mode ignores it).
...
`SqlParserService` populates `SqlParseResult.referencedTables` by walking each parsed `Statement` with `SqlStatementInspector`, a `TablesNamesFinder` subclass that also descends into window `PARTITION BY` expressions and a function call's aggregate `ORDER BY` / `LIMIT` / `HAVING` / keyword / named arguments (the finder skips these, so a `(SELECT … FROM secret)` hidden there would otherwise be missing from the set, and an incomplete set fails open), and that flags the embedded writes above in the same pass. CTE aliases are excluded automatically. If the walk throws on a `SELECT` / `INSERT` / `UPDATE` / `DELETE`, the statement is rejected with 422 `error.sql_analysis_failed`; only DDL / `OTHER` (gated by `can_ddl` / write access, not the allow-list) fall back to an empty set. Single-statement input yields the table set for that statement; a `BEGIN; …; COMMIT;` envelope yields the union across every inner statement. JSqlParser returns fully-qualified names via `Table.getFullyQualifiedName()` — `schema.table` when the writer qualified it, `table` when they didn't. The parser strips ASCII identifier quotes (`"`, `` ` ``, `[`, `]`) and ASCII-lowercases the result so admin-typed allow-list entries match user SQL regardless of quoting style.
