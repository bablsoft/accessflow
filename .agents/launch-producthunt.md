...
Written 2026-09-21. Starting position: new PH account, no supporter list, no email list, 6 GitHub stars, @AccessFlowIO under X's new-account reach limit.

## Recommendation

**Launch with v2.8.0 on Tuesday 2026-10-13** (the milestone's due date; Tue–Thu are the strongest PH days, and a launch goes live at 00:01 PT). That gives three weeks — the minimum to warm up a new PH account and build a supporter list from nothing.

v2.7.0 (due 2026-09-30) is nine days out: launching then means a cold account and no one lined up. If 2.8.0 slips, launch on the first Tuesday after the tag; the listing doesn't have to say "2.8", it's "AccessFlow".
...
| `website/og-image.png` says v2.4 | TODO | regenerate before launch — it's the image every share renders |
| 2.8.0 changelog entry | ships with the release | `website/changelog/` via prep-gh-release |

Known blemish in the video: the app header reads `v0.0.0` (e2e build) and the results show `display_name NULL` for the seeded users. Both are small; re-record after 2.8.0 if you want the real version string.
...
### Week 1 · Sep 22 – 28 · warm up and set up
...
- **X:** post the v2.6.0 release thread (unposted since Sep 15), pin the intro thread, start the 30-min daily engagement routine on the ~40 accounts listed this session.
- **GitHub:** pinned Discussion "AccessFlow launches on Product Hunt Oct 13 — feedback wanted"; a `## Launching` note in the README is optional.
...
- Regenerate `og-image.png`; decide voiceover vs captions and upload the video to YouTube (unlisted).

### Week 2 · Sep 29 – Oct 5 · line people up
- **DM wave 1** (personal, one at a time): "We launch on PH on Oct 13. No upvote ask — I'd value an honest comment or a question. Here's the 50-second demo." Track replies.
...
### Week 3 · Oct 6 – 12 · final approach
- Teaser thread on X (Oct 8–9): the demo GIF/clip + "launching Tuesday".
- Reminder DM wave 2 (Oct 12, short).
- Schedule the launch-day X posts; pre-write the maker comment and the ten answers.
- Tag v2.8.0 (release runs via `prep-gh-release`); confirm `version.json` and the changelog anchor are live.
- Sleep before 00:01 PT — that's 09:01 CEST if you're in Europe; plan the day accordingly.

## Launch day runbook (Tuesday 2026-10-13, times PT)
