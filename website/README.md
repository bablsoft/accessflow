...
> pillar cards under `#features`, the connector logo grid, one-command install tabs, seven
...
| [`docs/06-frontend.md`](../docs/06-frontend.md) "Setup wizard" + "SetupProgressWidget", [`docs/04-api-spec.md`](../docs/04-api-spec.md) `POST /auth/setup` and `GET /admin/setup-progress` | "First-time setup → Option A — Browser setup wizard" (`#first-run`) in [`docs/install/index.html`](docs/install/index.html) — the three wizard steps and the post-login onboarding checklist — **and** the step-by-step twin at `#guide-first-query-setup` in [`docs/guides/first-query/index.html`](docs/guides/first-query/index.html), which enumerates the same steps and drifts silently when only the install page is updated. Since [#898](https://github.com/bablsoft/accessflow/issues/898) the wizard asks which governance domains the org will use (`organizations.governs_apis` / `.governs_deployments`), and since [#926](https://github.com/bablsoft/accessflow/issues/926) the answer also decides which sidebar sections, review-queue tabs and dashboard widgets the app offers; both copies must keep saying that it shapes what is *shown*, not what is *allowed*, since that is the only thing a reader could mistake for an access control. The change-it-later half lives at `#cfg-organizations` in [`docs/configuration/users-roles/index.html`](docs/configuration/users-roles/index.html), which since #926 must also point an org admin at `/admin/governance-domains` rather than only at the platform-admin screen. Because #926 lets those sidebar entries disappear, the two guides that open by telling the reader to click them — [`docs/guides/api-governance/index.html`](docs/guides/api-governance/index.html) and [`docs/guides/deployment-approval/index.html`](docs/guides/deployment-approval/index.html) — each carry a "no such section in your sidebar?" prerequisite naming `/admin/governance-domains`; drop it and both guides dead-end on step one for a default install |
...
| [`docs/06-frontend.md`](../docs/06-frontend.md) "Service accounts admin pages (#875)", [`docs/07-security.md`](../docs/07-security.md) "Service accounts (#868, epic #867)" + "API-key authentication" (allow-list, rate limits, on-behalf-of), [`docs/13-mcp.md`](../docs/13-mcp.md) §1 + §4, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/admin/service-accounts`, [`docs/16-iac.md`](../docs/16-iac.md) bootstrap service accounts | "Service accounts" section (`#cfg-service-accounts`) in [`docs/configuration/users-roles/index.html`](docs/configuration/users-roles/index.html) — the `/admin/service-accounts` walkthrough (create → issue → rotate → tools → limits → principals → activity), the bootstrap read-only rule and where robots are badged; the three qa-items on [`ai-agents/index.html`](ai-agents/index.html) (`#guarantees`) point at the tabs by name and must keep matching them |
...
| [`docs/05-backend.md`](../docs/05-backend.md) `DefaultQueryExecutor` row-cap resolution (#934), [`docs/07-security.md`](../docs/07-security.md) "Per-table row-limit policies", [`docs/03-data-model.md`](../docs/03-data-model.md) `row_limit_policy`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/datasources/{id}/row-limit-policies` | The "Per-table row limits" item in "What the proxy enforces at execution" at `/features/database-access-governance/#proxy` + the "Row limits" paragraph under "Datasources" in [`docs/configuration/datasources/index.html`](docs/configuration/datasources/index.html) + the row-limits paragraph closing section 5 of [`docs/guides/datasource/index.html`](docs/guides/datasource/index.html) |
| [`docs/05-backend.md`](../docs/05-backend.md) "Per-user data-volume budgets (#942)", [`docs/07-security.md`](../docs/07-security.md) "Per-user data-volume budgets", [`docs/03-data-model.md`](../docs/03-data-model.md) `data_budgets` / `data_budget_usage`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/datasources/{id}/data-budgets`, [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_CORE_DATA_BUDGET_*` | The "Data budgets" subsection (`#cfg-data-budgets`) + the data-budget bullet of "Tune it" under "Datasources" in [`docs/configuration/datasources/index.html`](docs/configuration/datasources/index.html), the *data budget used (%)* operand under "Routing policies" in [`docs/configuration/review-workflows/index.html`](docs/configuration/review-workflows/index.html), and the *Data usage* item + "Manage data budgets" RBAC row in [`docs/configuration/users-roles/index.html`](docs/configuration/users-roles/index.html) |
...
| [`docs/05-backend.md`](../docs/05-backend.md) "Policy simulator (AF-630)", [`docs/04-api-spec.md`](../docs/04-api-spec.md) "Policy simulator" (the three `/simulate` endpoints), [`docs/07-security.md`](../docs/07-security.md) "Policy simulator", [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_POLICY_SIMULATION_*` env vars | the "Simulate it before you save it" paragraph + "What a simulation is, and is not" callout under "Routing policies" (`/docs/configuration/review-workflows/#cfg-routing-policies`), the matching pair under "Datasources" (`/docs/configuration/datasources/#cfg-datasources`, after the masking and row-security paragraphs), the "See what a rule would do before you save it" pair at `/features/database-access-governance/#review`, and the closing sentence + `Policy simulator` tag in the "Configurable review workflows" hub card (`/features/#database`). Plain-language rule: say *dry run against your own past queries*, never *A/B replay* or *corpus*; always keep the two honest caveats — nothing is saved and no database is touched, and queries an engine cannot classify offline are *unclassifiable*, never *unaffected* |
| [`docs/19-sql-review.md`](../docs/19-sql-review.md) (the feature chapter), [`docs/05-backend.md`](../docs/05-backend.md) "Deterministic SQL review rules", [`docs/03-data-model.md`](../docs/03-data-model.md) `sql_review_*` tables + `datasources.environment`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/sql-review/*` + `/admin/sql-review-rulesets`, [`docs/06-frontend.md`](../docs/06-frontend.md) "Live SQL review" + "SQL review rulesets", [`frontend/src/pages/admin/SqlReviewRulesetsPage.tsx`](../frontend/src/pages/admin/SqlReviewRulesetsPage.tsx) | the "SQL review rules" section (`#cfg-sql-review`) under Review workflows in [`docs/configuration/review-workflows/index.html`](docs/configuration/review-workflows/index.html) — the target of the app's *View docs* link on `/admin/sql-review`; the *Environment* sentence in the datasource wizard's "Configuration" step in [`docs/configuration/datasources/index.html`](docs/configuration/datasources/index.html); the editor-lint sentence under "Submitting a query" in [`docs/workflows/index.html`](docs/workflows/index.html); the "Rules that are written down" paragraphs at `/features/database-access-governance/#review`; the `SQL review rules` sentence + chip on the "Configurable review workflows" hub card (`/features/#database`); the "checked against your own SQL review rules" clause in the Databases pillar card on `/` (`#features`); the v2.6 sentence in `/roadmap/`'s milestone context; the `19 · SQL review rules` further-reading entry in [`docs/index.html`](docs/index.html) and the matching [`llms.txt`](llms.txt) line. **`BLOCK` escalates and never rejects** is stated on every surface that carries a sentence about severities (the review-workflows section, the workflows editor sentence, both `/features/` pages, `/roadmap/`) — change them together; the datasources sentence and the homepage clause only name the feature |
...
| [`docs/05-backend.md`](../docs/05-backend.md) "RAG knowledge base", [`docs/03-data-model.md`](../docs/03-data-model.md) `ai_config.rag_*`/`embedding_*` + `knowledge_document` + `vector_store`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/admin/ai-configs/{id}/knowledge-documents` + `/rag/test`, [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_RAG_*` + "pgvector for RAG" | "RAG knowledge base" sentence + `RAG` chip in the "AI query analysis" hub card (`/features/#database`) + the analyzer section at `/features/database-access-governance/#ai` + "RAG knowledge base" paragraph, `ai-configs-rag` (light + dark) figure, the "Voyage AI for an Anthropic install" callout (embedding-only provider, its own API key, the 256/512/1024/2048 widths vs `ACCESSFLOW_RAG_PGVECTOR_DIMENSIONS`), and the pgvector callout under "AI configurations" in [`docs/configuration/ai/index.html`](docs/configuration/ai/index.html), plus the embeddings-vs-analysis callout under the provider table in [`docs/guides/ai-analysis/index.html`](docs/guides/ai-analysis/index.html). The embedding-provider list also reaches two marketing surfaces: the `Voyage AI embeddings` item in the hero meta strip of [`index.html`](index.html) and both the embeddings sentence and the `Voyage AI embeddings` chip in the "AI query analysis" hub card of [`features/index.html`](features/index.html) — those two enumerate *analysis* providers everywhere else, so keep the embedding-only framing explicit or the page starts implying Voyage can analyze queries |
...
| [`docs/05-backend.md`](../docs/05-backend.md) "Job execution monitoring (#923)", [`docs/04-api-spec.md`](../docs/04-api-spec.md) "Platform Jobs", [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_SCHEDULING_EXECUTIONS_*` | "Scheduled jobs" section (`#cfg-job-monitoring`) in [`docs/configuration/users-roles/index.html`](docs/configuration/users-roles/index.html) — the platform-admin `/admin/jobs` page: the table, the history drawer, the Running / Abandoned / Not run yet states, lock-skipped ticks not being recorded, the read-only rule and the retention defaults (14 days, 500 runs per job) |
...
| [`docs/18-deployment-governance.md`](../docs/18-deployment-governance.md), [`docs/03-data-model.md`](../docs/03-data-model.md) `deployment_pipelines` / `deployment_environments` / `deployment_freeze_windows` / `deployment_requests`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) "Deployment Governance", [`docs/05-backend.md`](../docs/05-backend.md) "Deployment Governance (deploygov module)", [`docs/07-security.md`](../docs/07-security.md) "Deployment governance security" (epic AF-682) | "Gated CI/CD releases" hub card (`/features/#deployment`) — renamed from "Deployment approval governance" by [#787](https://github.com/bablsoft/accessflow/issues/787), which moved that phrase to the `/features/deployment-governance/` spoke as its head term — plus that spoke, `/`'s "Deployments" pillar card and the `/use-cases/#deploy` persona; the homepage `#docs` grid card was deleted, not moved, **"Deployment pipelines"** section (`/docs/configuration/review-workflows/#cfg-deployment-pipelines`), incl. the environment-labels and version-matrix paragraphs derived from [`docs/18-deployment-governance.md`](../docs/18-deployment-governance.md) §9 and [`docs/06-frontend.md`](../docs/06-frontend.md) `DeploymentVersionsPage` (#743), and the environment **Order** / **Database** fields (#877, [`docs/18-deployment-governance.md`](../docs/18-deployment-governance.md) §1) described in step 4 of the [deployment-approval guide](docs/guides/deployment-approval/index.html) + matching WebP under [`images/docs/`](images/docs/) (`deployments-list`, `deployment-detail`, `deployment-reviews-queue`, `deployment-rollback-reviews`, `deployment-pipelines-list`, `deployment-pipeline-environments`, `deployment-pipeline-freeze-windows`, `deployment-pipeline-ci`, light + dark) |
| [`docs/13-mcp.md`](../docs/13-mcp.md) (§1 API keys and service accounts, §3 the twelve tools, §4 limits, errors, the #872 tool allow-list and the unfiltered `tools/list`), [`docs/07-security.md`](../docs/07-security.md) "API-key authentication", [`docs/05-backend.md`](../docs/05-backend.md) "MCP server (mcp module)", [`README.md`](../README.md) MCP bullet, [`docs/06-frontend.md`](../docs/06-frontend.md) "Service accounts admin pages (#875)" | **"AI agents"** topic page ([`ai-agents/index.html`](ai-agents/index.html)) — `#how` (the five-step flow), `#guarantees` (the qa-grid; every item names its enforcement point, and the tool allow-list is the one that runs in the tool wrapper rather than the service layer), `#setup` (Claude Desktop / Claude Code snippets). Says "twelve tools" and "sees twelve tools" deliberately: the allow-list narrows what a service account may *call*, never what `tools/list` advertises |
| [`CLAUDE.md`](../CLAUDE.md) "Project at a Glance", [`docs/02-architecture.md`](../docs/02-architecture.md), [`docs/04-api-spec.md`](../docs/04-api-spec.md) (API-key auth, `/queries` lifecycle), [`docs/13-mcp.md`](../docs/13-mcp.md), [`docs/16-iac.md`](../docs/16-iac.md), [`docs/12-roadmap.md`](../docs/12-roadmap.md) (the wire-protocol gateway is **roadmap**, not product), [`connectors/*/connector.json`](../connectors/) `name` (the help-corpus generator fails when a connector is missing from the engine list) | **"Integrations &amp; boundaries"** chapter ([`docs/integrations/index.html`](docs/integrations/index.html)) — `#fit`, `#ways-in`, `#not-a-driver` (incl. the ODBC / ADO.NET Q&amp;A), `#supported`, `#deployment-shape`, `#data`, `#governs`. Written for the in-app help agent as much as for visitors: it is the one page that states what AccessFlow is **not** (no ODBC/JDBC/ADO.NET driver, no database wire protocol, no SaaS) and marks its engine / sign-in / AI-provider lists as **complete**, which is the wording the help prompt keys on to answer "not supported" instead of "I don't know". Never say "connect to AccessFlow" on this page without saying how |
...
| [`docs/02-architecture.md`](../docs/02-architecture.md), [`docs/05-backend.md`](../docs/05-backend.md) (JIT access, break-glass AF-385, routing policies AF-379, masking, lifecycle AF-499, attestation AF-384, compliance AF-459, apigov AF-500/AF-518), [`docs/07-security.md`](../docs/07-security.md), [`docs/17-api-governance.md`](../docs/17-api-governance.md) | **"Use cases" section** (homepage, `#use-cases`) — one-line persona tiles since [#789](https://github.com/bablsoft/accessflow/issues/789), seven of them since epic [#682](https://github.com/bablsoft/accessflow/issues/682) added `#deploy`, each linking a `/use-cases/` anchor (`#platform`, `#sre`, `#dba`, `#compliance`, `#privacy`, `#api`, `#deploy`); the full persona stories live on `/use-cases/` |
...
| The latest **stable** GitHub release (tag, publish date) | **`version.json`** ([`version.json`](version.json)) — `{"version", "released_at", "changelog_url"}`. Read by every self-hosted backend at most once a day (`GET /api/v1/system/update-status` → the sidebar version chip); a `-beta.N` tag must **not** be written here. Also updated by `prep-gh-release` step 4a′ |
...
| [`docs/09-deployment.md`](../docs/09-deployment.md) "Remote corpus refresh, and why it is off" (`ACCESSFLOW_HELP_CORPUS_*`) + [`docs/14-connectors.md`](../docs/14-connectors.md) "Persistence and air-gap", [`charts/accessflow/examples/values-airgapped.yaml`](../charts/accessflow/examples/values-airgapped.yaml) | the "What else calls out on its own" paragraph under "Running AccessFlow" in [`docs/install/index.html`](docs/install/index.html) — the page's egress answer, and the page's egress answer for the in-app help assistant (the feature itself is the guide row below). It names three switches; adding a fourth self-initiated outbound call means editing this paragraph, the two docs sources and the air-gapped values example together, or the site's egress claim silently becomes wrong |
| [`docs/05-backend.md`](../docs/05-backend.md) "Help agent configuration" / "Help corpus loading, indexing and re-ingestion" / "Help chat runtime" (epic [#899](https://github.com/bablsoft/accessflow/issues/899)), [`docs/04-api-spec.md`](../docs/04-api-spec.md) `/admin/help-agent` + `/help-chat/*`, [`docs/06-frontend.md`](../docs/06-frontend.md) "In-app help chat", [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_HELP_AGENT_*` + `ACCESSFLOW_HELP_CORPUS_*` and "Budget the tokens before you enable it", [`help-corpus/README.md`](../help-corpus/README.md), plus the real UI labels in [`frontend/src/locales/en.json`](../frontend/src/locales/en.json) `admin.help_agent.*` / `help_chat.*` | The **Ask the help assistant** guide ([`docs/guides/help-assistant/`](docs/guides/help-assistant/), anchored `guide-help-assistant*` — the `docsAnchor` of `/admin/help-agent`), the reciprocal "In-app help assistant" reference section (`#cfg-help-assistant`) in [`docs/configuration/ai/index.html`](docs/configuration/ai/index.html), the "Who do I ask?" entry under `#questions` on the homepage, the help-assistant sentence and chip on the "AI query analysis" hub card (`/features/`), and the v2.6 in-development paragraph on [`roadmap/index.html`](roadmap/index.html). The guide is the page that says *start it locally, bind an AI provider, ask* — the pitch for new users, admins and evaluators alike — and it is itself a corpus source, so the assistant can explain the assistant |
...
| [`frontend/src/pages/editor/QueryEditorPage.tsx`](../frontend/src/pages/editor/QueryEditorPage.tsx), [`frontend/src/pages/reviews/ReviewHubPage.tsx`](../frontend/src/pages/reviews/ReviewHubPage.tsx) (the unified review queue, #772) — end-user SPA pages | [`docs/workflows/index.html`](docs/workflows/index.html) — the "End-user workflows" chapter (Submitting / Scheduling a query, Drafting queries from natural language, Suggested queries, Query templates library, Reviewing &amp; bulk approval) + matching screenshots under [`images/docs/`](images/docs/) (`editor`, `editor-text-to-sql`, `editor-schedule`, `editor-query-templates`, `queries-list`, `reviews-queue`, `reviews-queue-bulk`, light + dark) |
...
├── version.json     # Release manifest — latest stable version + changelog anchor; polled daily by self-hosted installs (#836)
...
├── use-cases/       # Topic page — the seven persona use cases and their mocks (AF-785)
├── roadmap/         # Topic page — available-now grid, milestone context, planned band (AF-786)
├── changelog/       # Topic page — one anchored section per stable release, newest first (#836)
...
│   ├── guides/      #   Task-oriented setup manuals (AF-773) — hub + 10 guides:
...
│   └── integrations/ #  every way in, what AccessFlow is not, complete engine / sign-in / AI lists
├── images/
...
ten task-oriented manuals), and **Reference** (the eight configuration chapters, end-user
...
arrive at; the remaining ten chapters, the eight under `/docs/configuration/`, `/docs/iac/` and
`/docs/integrations/`, at 0.7 alongside the ten individual guides, since each answers a question only some deployments
ask; `/roadmap/` and `/changelog/` at 0.6, since they report
