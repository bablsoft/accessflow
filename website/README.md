...
| [`docs/05-backend.md`](../docs/05-backend.md) "Approval-outcome prediction" (AF-645), [`docs/03-data-model.md`](../docs/03-data-model.md) `approval_prediction_model` / `approval_predictions`, [`docs/04-api-spec.md`](../docs/04-api-spec.md) `approval_prediction` block + `approval_probability` + `query.prediction_complete`, [`docs/09-deployment.md`](../docs/09-deployment.md) `ACCESSFLOW_AI_APPROVAL_PREDICTION_*` env vars | "historical approval likelihood" blurb + `Approval likelihood (advisory)` tag in the "AI query analysis" feature tile + the approval-likelihood bullet in the "Review at scale without a ticket queue" use case + "Approval-likelihood prediction" item in the Planned roadmap group (homepage) + the "Approval-likelihood prediction" subsection (`#cfg-approval-prediction`) in [`docs/configuration/ai/index.html`](docs/configuration/ai/index.html) and the queue paragraph under "Reviewing &amp; bulk approval" in [`docs/workflows/index.html`](docs/workflows/index.html). Keep the advisory-only framing — it is a triage signal, never auto-approval |
