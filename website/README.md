...
| [`docs/13-mcp.md`](../docs/13-mcp.md) (§1 API keys and service accounts, §3 the twelve tools, §4 limits, errors, the #872 tool allow-list and the unfiltered `tools/list`), [`docs/07-security.md`](../docs/07-security.md) "API-key authentication", [`docs/05-backend.md`](../docs/05-backend.md) "MCP server (mcp module)", [`README.md`](../README.md) MCP bullet | **"AI agents"** topic page ([`ai-agents/index.html`](ai-agents/index.html)) — `#how` (the five-step flow), `#guarantees` (the qa-grid; every item names its enforcement point, and the tool allow-list is the one that runs in the tool wrapper rather than the service layer), `#setup` (Claude Desktop / Claude Code snippets). Says "twelve tools" and "sees twelve tools" deliberately: the allow-list narrows what a service account may *call*, never what `tools/list` advertises |
