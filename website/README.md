...
> spoke pages. Rows below that still name a homepage *feature tile*, a homepage *roadmap
> available-now group*, or a homepage `#use-cases` row now resolve to the corresponding copy
> on `/features/` (hub card + spoke), `/roadmap/`, and `/use-cases/`; re-pointing them row by
> row is [#792](https://github.com/bablsoft/accessflow/issues/792)'s audit sweep.
...
| [`docs/12-roadmap.md`](../docs/12-roadmap.md) | **Roadmap stub** (homepage, `#roadmap`) — since [#789](https://github.com/bablsoft/accessflow/issues/789) a two-line stub that keeps the `#roadmap` id alive (it is in the nav of every deployed page and in the wild) and links the `/roadmap/` page, which owns the capability map |
...
| [`docs/`](../docs/) chapter filenames + H1s (01–18, incl. [`docs/14-connectors.md`](../docs/14-connectors.md), the [`docs/15-engine-sdk.md`](../docs/15-engine-sdk.md) engine-author guide, the [`docs/16-iac.md`](../docs/16-iac.md) IaC guide, the [`docs/17-api-governance.md`](../docs/17-api-governance.md) API-governance guide, and the [`docs/18-deployment-governance.md`](../docs/18-deployment-governance.md) deployment-governance guide) | The further-reading links inside [`docs/index.html`](docs/index.html), and the footer "Docs" column — which [#791](https://github.com/bablsoft/accessflow/issues/791) repointed at the on-site chapters (`/docs/`, `/docs/install/`, `/docs/configuration/auth/`, `/docs/workflows/`, `/docs/iac/`), leaving exactly one engineering-reference link to `github.com/blob` for [`docs/04-api-spec.md`](../docs/04-api-spec.md), the one of the four with no on-site equivalent. The homepage `#docs` grid of raw `github.com/blob` cards was **deleted** (not moved) by [#789](https://github.com/bablsoft/accessflow/issues/789); `/docs/` is the docs hub |
...
— not homepage fragments, so each one is a URL Google can follow and rank. `Quick start` and
`Get AccessFlow` stay in `.nav-right`; `How it works` and `Questions` live in the footer, one
scroll from the top of `/`. There is deliberately **no dropdown**: it would need new `app.js`
on a site whose CSP forbids inline script, and a crawler cannot follow one — the `/features/`
sidebar is the discoverability affordance instead. Each page marks its own nav item with
`aria-current="page"` and `nav-link-active`, resolved by **longest URL prefix**, so the three
`/features/*` spokes light up `Features` and every `/docs/**` chapter lights up `Docs`. `/`
and `/ai-agents/` are not nav items and mark nothing.
