...
| [`docs/14-connectors.md`](../docs/14-connectors.md), [`connectors/`](../connectors/) catalog, [`docs/04-api-spec.md`](../docs/04-api-spec.md) Connector endpoints, [`docs/03-data-model.md`](../docs/03-data-model.md) `datasources.connector_id` | Dedicated **"Connectors" section** (homepage, `#connectors`) — logo grid of the catalog (logos in [`db-icons/`](db-icons/), copied from `connectors/<id>/logo.svg`); "Connector catalog" feature tile; ClickHouse + "connector catalog" in supported-DB strips (architecture diagram, proxy tag, tech-stack targets); "Declarative connector catalog" roadmap item; + the "Connectors" section (built-in connector list + install how-to) under Configuration in [`docs/index.html`](docs/index.html) |
