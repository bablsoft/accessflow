...
- **Result-export governance & DLP** — masking governs what a user *sees*; export policies govern what *leaves*. Per-datasource policies with role / group / user targeting (`allow` / `watermark` / `row-cap` / `deny-when-classified`, most-restrictive-wins, **no admin bypass**) gate the signed CSV/PDF result-export endpoint and the results attachment on recurring-run emails. Classified detection reuses the classification tags at export time; watermarked exports carry the exporter, timestamp, and query id **inside the signed bytes** (stripping the stamp invalidates the signature); every export writes a `RESULT_EXPORTED` audit row, and classified exports notify org admins — so "who downloaded this data" always has an answer.
...
- **AI query analysis** — pluggable adapters for OpenAI, Anthropic Claude, self-hosted Ollama, any OpenAI API–compatible backend (vLLM, LM Studio, Together, Groq, OpenRouter, …) via a custom endpoint, and Hugging Face (Inference Providers router or a local / self-hosted TGI server); risk scoring (0–100), missing-index detection, anti-pattern hints. Embeddings for the RAG knowledge base are configured separately on the same row and draw on a different list — **Voyage AI** belongs there and nowhere else, which gives an Anthropic-only install an embedding path without a second chat vendor (Claude has no embeddings API). Per-organization configuration via the admin UI, including an **editable analyzer prompt** per configuration (override the built-in template, or leave it blank to use the default); admin **AI analyses dashboard** charts average risk over time, top issue categories, and most active submitters. A per-organization **rate limit** (requests/minute, default 30) and optional **monthly token budget** cap AI usage so a runaway editor or compromised account can't drain the provider API key — enforced before every analysis call; exceedance returns HTTP 429 in the editor or records a sentinel `CRITICAL` analysis row on the async path. A per-organization **fallback pool** marks configurations (typically a local Ollama) to retry in priority order when the primary provider is unreachable — air-gap-friendly resilience without weakening any failure semantics.
...
- **In-app help assistant (#899)** — have a question about AccessFlow? Ask AccessFlow. A launcher in the app shell opens a chat panel that answers "how do I submit a query for review?", "what does break-glass do?", "how do I connect Snowflake?" from the product's **own documentation, bundled with the running build** — so a new user or an admin can start it locally, bind any AI configuration (a hosted provider or a local Ollama), and interrogate the application itself instead of the website. Answers cite the documentation sections they used as links the **server** resolves (the model only emits `[n]`). It is a **documentation reader, not a data surface**: no tools, no actions, no query, result, audit row, schema or datasource ever reaches the prompt — at most the current screen's label and the caller's permission names, and that is switchable off. With an embedding provider and vector store on the bound configuration the corpus is indexed per organization (kept invisible to the RAG knowledge base) and re-indexed on upgrade; without one — including Anthropic-only installs — a bundled quick reference answers, uncited. Conversations are private, retention-pruned, rate-limited per user and counted against the monthly AI token budget; remote corpus refresh is opt-in and `ACCESSFLOW_HELP_CORPUS_OFFLINE` pins an air-gapped install to what it shipped with. Guide: [Ask the help assistant](https://accessflow.io/docs/guides/help-assistant/).
...
- **Automatic query suggestions (#776)** — the editor's right rail offers whole draft queries mined from queries your organisation has *already had approved* on that datasource, so an analyst does not start from a blank buffer. Each one shows the evidence behind it — how often it was approved, by how many people, how recently — and **Apply as draft** fills the editor. Ranking is a deterministic local heuristic (frequency × recency × overlap with your own recent tables), so there is no AI provider call and the rail keeps working with AI switched off. Every suggestion is filtered against **your own** effective permission before it reaches you, so it can never hint at data you cannot reach; break-glass and recurring runs never enter the corpus; and an applied draft is still analysed and reviewed like any other query, recorded in the audit trail with its own provenance.
...
- **Recurring approved queries** — "run this every Monday 08:00 until \<date\>" as a single reviewable request: the query, cadence (cron in UTC or a fixed interval), and mandatory expiry are approved **once**, then a clustered-safe scheduler executes each occurrence through the full proxy pipeline with **current** policy state, recording every run as its own immutable request (results, snapshot, audit). The submitter or any reviewer can cancel the series, and permission revocation, an inactive datasource, or unparseable SQL **halts it fail-closed** with an audited reason. Occurrence results are delivered over your notification channels — email carries the rows as a capped `results.csv` attachment, chat channels a summary with a link — turning AccessFlow into a governed lightweight report scheduler.
...
- **Least-privilege intelligence (#625)** — AccessFlow already knows who ran what, so it tells you which standing grants nobody is using. A clustered-safe background job folds executed-query and executed-API-call audit metadata into a per-grant usage summary — last used, how often, and which of the granted tables / operations were ever actually touched — for both datasource and API-connector grants, and labels each one `NEVER_USED`, `STALE`, `OVER_SCOPED`, `ACTIVE`, or `INSUFFICIENT_DATA`. Admins and auditors get a standing **over-provisioned access** report at `/admin/over-provisioned-access` (filter by resource, holder, or recommendation; export as CSV), attestation reviewers see the label on every campaign item with the worklist sorted staleness-first so never-used grants land on page one, and an opt-out **stale-grant notification** nudges org admins when a grant crosses the staleness threshold. **Advisory only**: nothing is ever revoked automatically, and a recommendation is never an input to routing, grant coverage, or any other decision path.
- **Effective-access explainer (#859)** — ask *why* without submitting a probe query in production. A read-only trace endpoint replays one hypothetical request through the very evaluators that decide real ones — datasource and quota gates, the merged effective permission with each contributing grant named, every routing policy in priority order (including the ones that nearly matched), the grant-covered fast path, the review plan, who could review it, and the row-security and masking policies that would apply — and answers per stage with the terminal status. Nothing is written, executed, published or sent to an AI provider, and the approximations a hypothetical request cannot avoid are named in the response. Alongside it, the reverse index: *who can write to this table* — one row per user, naming every source that grants it (direct row, inherited group permission, active JIT grant, break-glass, and the `QUERY_ADMIN` holders who bypass the per-datasource gate entirely). Both live in the app at `/admin/access-simulations` (#1066): a *Query trace* tab that lays the trace out stage by stage — skipped stages included, every routing policy in a table with the decisive one highlighted — and a *Who has access* tab that auditors can use too.
...
- **MCP server** — built-in Spring AI MCP server exposes a stateless tool surface so external AI agents can submit queries through the same review pipeline, and also discover schemas, validate SQL without executing it, read masking- and row-security-aware sample data, monitor their queries, and review their own audit trail. Service accounts are managed from the **Service accounts** admin page (create, show-once key issuance, graced rotation, tool allow-list, rate limits, on-behalf-of principals, activity), and are badged wherever people are listed. A service account's key can be restricted to an explicit tool allow-list, enforced on every call, and every API-key request is rate-limited per identity (429 + `Retry-After`). An agent can also name the person it acts *for* (`X-AccessFlow-On-Behalf-Of`, after that person consents): its permissions stay its own, the person is stamped on the submission and on every audit row of the call, and can no longer approve that submission themselves.
- **Connector catalog** — supported databases are described declaratively in a repo-root `connectors/` folder (one manifest + logo per connector), not hardcoded. Each connector carries a `category` (`RELATIONAL` for the SQL family; `WAREHOUSE` for the cloud data warehouses; `DOCUMENT`, `KEY_VALUE`, `WIDE_COLUMN`, `SEARCH`, or `GRAPH` for the NoSQL umbrella) and the marketplace groups them into SQL, cloud data-warehouse, and NoSQL sections accordingly. Admins browse the **Connectors** marketplace and install a relational database's JDBC driver with one click (downloaded + SHA-256-verified + cached); engines beyond the built-in five (e.g. ClickHouse) install the same way. **MongoDB**, **Couchbase**, **Redis**, **Apache Cassandra**, **ScyllaDB**, **Elasticsearch**, **OpenSearch**, **Amazon DynamoDB**, **Neo4j**, **Snowflake**, **Google BigQuery**, and **Databricks SQL** are native (non-JDBC) engine-managed connectors whose engines ship as on-demand **plugin JARs** (`engines/mongodb/`, `engines/couchbase/`, `engines/redis/`, `engines/cassandra/` — the Cassandra plugin serves both Cassandra and ScyllaDB; `engines/elasticsearch/` — the Elasticsearch plugin serves both Elasticsearch and OpenSearch; `engines/dynamodb/`; `engines/neo4j/`; `engines/snowflake/`; `engines/bigquery/`; `engines/databricks/`), downloaded + verified through the same catalog pipeline. The catalog ships in the image and is also published as a release artifact.
...
- **Cloud data warehouses — Snowflake, Google BigQuery, Databricks SQL** — first-class `WAREHOUSE` connectors (the eighth, ninth, and tenth engine plugins: `engines/snowflake/`, `engines/bigquery/`, `engines/databricks/`) whose native auth models replace host/port/username/password: Snowflake authenticates with a password **or** a key-pair PKCS#8 private key — encrypted (passphrase-protected) or not — against the account host and opens billed warehouse sessions per request only; BigQuery connects with a service-account key JSON + GCP project (optionally `project.dataset`); Databricks talks to the SQL Statement Execution REST API with a personal access token against a workspace host + warehouse HTTP path, and falls back to the `EXTERNAL_LINKS` disposition for result sets above the inline ~25 MiB cap — streaming the presigned chunks up to the proxy's row and byte caps, never persisting the URLs. All three classify warehouse SQL onto the same approval workflow (scripting, data-movement, and maintenance commands like `COPY INTO` / `OPTIMIZE` / `VACUUM` are rejected up front), splice row-level security into the WHERE clause with bound parameters (failing closed on shapes that cannot be filtered), and apply field masking post-fetch — BigQuery recursively across nested RECORD fields.
...
- **Deployment approval governance (epic AF-682)** — gate CI/CD **deployments** behind the same submit → AI → review → approve machinery. Admins define **deployment pipelines** (GitHub Actions, GitLab CI, Azure Pipelines, Jenkins, CircleCI, Bitbucket Pipelines, or generic) with ordered per-environment policies (required approvals, review-plan overrides, break-glass opt-in) and per-user/per-group `can_trigger` / `can_break_glass` grants. A CI job triggers a deployment request over the REST API (API-key auth, idempotent on the CI run id), AI analyzes the release metadata, routing policies (environment / provider / version glob / risk / time window) can auto-approve, auto-reject or escalate, and the pipeline polls a **fail-closed deployment gate** until every approval is granted — with **freeze windows** (one-off or weekly recurring, `HOLD` or auto-`REJECT`), scheduled deploys, audited break-glass, post-deploy **outcome reporting** (success / failure / rollback) and a mandatory rollback retro-review. Native GitHub Action / GitLab CI / Azure Pipelines wrappers reduce the CI glue to one step, and the web UI ships the full surface: a deployment list + detail view (risk, approvals, releasability banner, outcome timeline), a reviewer queue with a rollback-review worklist, and pipeline administration incl. a copy-paste CI setup panel. Past the gate, AccessFlow tracks **which version is running where**: each environment carries free-form tags (customer / region / tier) and a current+previous version updated inside the execution and outcome transactions, surfaced as an org-wide **version matrix** with per-row drift ("N versions / D days behind" against the pipeline's newest successful release — read-time string comparison, no semver, no alerting), a per-environment deployment-history timeline, and drift badges on the deployment list and detail pages. See [docs/18-deployment-governance.md](https://github.com/bablsoft/accessflow/blob/main/docs/18-deployment-governance.md).
...
- **Release update hint (#836)** — every signed-in user sees the running version under the sidebar logo, and when a newer stable release exists it becomes a link straight to that release's entry on the public [changelog](https://accessflow.io/changelog/). The check is one unauthenticated `GET` for a small static manifest, made at most once a day and off the request path — no install identifier, no telemetry — and `ACCESSFLOW_UPDATES_ENABLED=false` switches it off entirely for air-gapped installs.
- **Infrastructure as Code** — an official **Terraform / OpenTofu provider** (`bablsoft/accessflow`) and reusable **GitHub Actions** + **GitLab CI / Azure Pipelines templates** manage datasources, review plans, routing / row-security / masking policies, SQL review rulesets, AI configs, and notification channels declaratively over the REST API. Pipelines authenticate with a bootstrap-provisioned **service-account API key**. See [`docs/16-iac.md`](https://github.com/bablsoft/accessflow/blob/main/docs/16-iac.md).
...
| AI backends | OpenAI, Anthropic, Ollama, any OpenAI-compatible endpoint, Hugging Face (Inference Providers router or local TGI) for analysis; OpenAI, Ollama, OpenAI-compatible, Hugging Face, Voyage AI for embeddings (admin-configurable per organization) |
...
| Infrastructure as Code | Official Terraform / OpenTofu provider (Go, terraform-plugin-framework) + reusable GitHub Actions and GitLab CI / Azure Pipelines templates (incl. a deployment-approval gate) |
...
The setup wizard creates the first organization and admin user, asks which governance domains you plan to use — database access governance is always on, while outbound API calls and CI/CD deployments are optional. The answer decides which navigation, review tabs and dashboard widgets you are shown (never what anyone may access) and an admin can change it later at `/admin/governance-domains`. The wizard also optionally configures SMTP — no `.env`, no key generation, no Maven, no npm.
...
│   │   ├── schemachange/     # Governed DDL change sets — authoring + the DDL validation gate (#879), promotion along a pipeline's environment ladder (#880) and the scheduled schema drift job (#881)
