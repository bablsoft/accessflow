 ...
 * {@code Pageable}/{@code Page} inside the attestation module (core's PageAdapter is module-private).
 */
final class AttestationPageAdapter {

    private AttestationPageAdapter() {
    }
    ...
    private static Sort toSpringSort(PageRequest request) {
        ...
        return Sort.by(request.sort().stream().map(AttestationPageAdapter::toSpringOrder).toList());
    }
