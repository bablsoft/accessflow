package com.bablsoft.accessflow.audit.api;

/**
 * Catalog of audit actions written to {@code audit_log.action}. Mirrors the table in
 * {@code docs/03-data-model.md}; {@link #QUERY_AI_FAILED} is an extension that lets the read API
 * filter for failed AI runs without parsing the JSONB metadata.
 */
public enum AuditAction {
    QUERY_SUBMITTED,
    QUERY_AI_ANALYZED,
    QUERY_AI_FAILED,
    QUERY_AI_REANALYZE_REQUESTED,
    QUERY_REVIEW_REQUESTED,
    QUERY_APPROVED,
    QUERY_REJECTED,
    QUERY_EXECUTED,
    QUERY_BREAK_GLASS_EXECUTED,
    BREAK_GLASS_REVIEWED,
    QUERY_FAILED,
    QUERY_CANCELLED,
    RECURRING_SERIES_HALTED,
    DATASOURCE_CREATED,
    DATASOURCE_UPDATED,
    DATASOURCE_REPLICA_FALLBACK,
    DATASOURCE_SECRET_RESOLVED,
    DATASOURCE_SECRET_RESOLUTION_FAILED,
    PERMISSION_GRANTED,
    PERMISSION_REVOKED,
    PERMISSION_GROUP_GRANTED,
    PERMISSION_GROUP_REVOKED,
    REVIEW_PLAN_CREATED,
    REVIEW_PLAN_UPDATED,
    REVIEW_PLAN_DELETED,
    /** #622: a reviewer delegated their review duty for an out-of-office window. */
    REVIEW_DELEGATION_CREATED,
    REVIEW_DELEGATION_REVOKED,
    USER_LOGIN,
    USER_LOGIN_FAILED,
    USER_LOGIN_TOTP_FAILED,
    USER_CREATED,
    USER_DEACTIVATED,
    USER_PROFILE_UPDATED,
    USER_PASSWORD_CHANGED,
    USER_PASSWORD_RESET_REQUESTED,
    USER_PASSWORD_RESET_COMPLETED,
    USER_TOTP_ENABLED,
    USER_TOTP_DISABLED,
    ROLE_CREATED,
    ROLE_UPDATED,
    ROLE_DELETED,
    API_KEY_CREATED,
    API_KEY_UPDATED,
    AI_CONFIG_CREATED,
    AI_CONFIG_UPDATED,
    AI_CONFIG_DELETED,
    KNOWLEDGE_DOCUMENT_CREATED,
    KNOWLEDGE_DOCUMENT_DELETED,
    CUSTOM_DRIVER_UPLOADED,
    CUSTOM_DRIVER_DELETED,
    CONNECTOR_INSTALLED,
    USER_INVITED,
    USER_INVITATION_RESENT,
    USER_INVITATION_REVOKED,
    USER_INVITATION_ACCEPTED,
    SYSTEM_SMTP_UPDATED,
    SYSTEM_SMTP_DELETED,
    SYSTEM_SMTP_TEST_SENT,
    ORGANIZATION_CREATED,
    NOTIFICATION_CHANNEL_CREATED,
    NOTIFICATION_CHANNEL_UPDATED,
    NOTIFICATION_CHANNEL_DELETED,
    NOTIFICATION_DELIVERY_EXHAUSTED,
    TICKET_CREATED,
    TICKET_STATUS_SYNCED,
    OAUTH2_CONFIG_UPDATED,
    SAML_CONFIG_UPDATED,
    LANGFUSE_CONFIG_UPDATED,
    HELP_AGENT_CONFIG_UPDATED,
    AUDIT_LOG_EXPORTED,
    AUDIT_SINK_CREATED,
    AUDIT_SINK_UPDATED,
    AUDIT_SINK_DELETED,
    USER_GROUP_CREATED,
    USER_GROUP_UPDATED,
    USER_GROUP_DELETED,
    USER_GROUP_MEMBER_ADDED,
    USER_GROUP_MEMBER_REMOVED,
    DATASOURCE_REVIEWER_ADDED,
    DATASOURCE_REVIEWER_REMOVED,
    QUERY_TEMPLATE_CREATED,
    QUERY_TEMPLATE_UPDATED,
    QUERY_TEMPLATE_DELETED,
    QUERY_TEMPLATE_RESTORED,
    SLACK_APP_CONFIG_UPDATED,
    SLACK_APP_CONFIG_DELETED,
    ACCESS_REQUEST_SUBMITTED,
    ACCESS_REQUEST_APPROVED,
    ACCESS_REQUEST_REJECTED,
    ACCESS_REQUEST_CANCELLED,
    ACCESS_GRANT_EXPIRED,
    ACCESS_GRANT_REVOKED,
    MASKING_POLICY_CREATED,
    MASKING_POLICY_UPDATED,
    MASKING_POLICY_DELETED,
    ROUTING_POLICY_CREATED,
    ROUTING_POLICY_UPDATED,
    ROUTING_POLICY_DELETED,
    ROUTING_POLICY_REORDERED,
    SQL_REVIEW_RULESET_CREATED,
    SQL_REVIEW_RULESET_UPDATED,
    SQL_REVIEW_RULESET_DELETED,

    /**
     * Admin lifecycle of a service account and of the API keys issued on its behalf via
     * {@code /admin/service-accounts} (#871). Resource type {@code SERVICE_ACCOUNT}, resource id
     * = the account's user id. Accounts reconciled from bootstrap YAML keep auditing as
     * {@code API_KEY_CREATED} / {@code API_KEY_UPDATED} with {@code source = BOOTSTRAP}.
     */
    SERVICE_ACCOUNT_CREATED,
    SERVICE_ACCOUNT_UPDATED,
    SERVICE_ACCOUNT_DEACTIVATED,
    SERVICE_ACCOUNT_KEY_ISSUED,
    SERVICE_ACCOUNT_KEY_ROTATED,
    SERVICE_ACCOUNT_KEY_REVOKED,
    /** A human → service-account on-behalf-of grant was created / revoked (#874). Resource id = the account. */
    SERVICE_ACCOUNT_DELEGATION_GRANTED,
    SERVICE_ACCOUNT_DELEGATION_REVOKED,

    /**
     * A {@code BLOCK} SQL review finding suppressed an auto-approve path and forced the request to
     * human review (#864). System-attributed — null actor, {@code trigger=sql_review}. Written only
     * when the guard changed the outcome; never for {@code WARN}, never on a rejection.
     */
    SQL_REVIEW_BLOCKED,
    /**
     * The bytes-scanned cap (#941) changed a query's outcome: refused it when it left
     * {@code PENDING_AI} ({@code stage=decision}), turned an automatic approval into human review,
     * or failed it just before execution ({@code stage=execution}). System-attributed — null actor,
     * {@code trigger=bytes_scanned_cap}; metadata carries {@code limit}, {@code source},
     * {@code estimated_bytes} and {@code outcome}.
     */
    QUERY_BYTES_SCANNED_CAP_ENFORCED,
    /**
     * An exhausted data-volume budget (#942) refused a query or turned an automatic approval into
     * human review ({@code stage=decision}), or refused an execution ({@code stage=execution}).
     */
    QUERY_DATA_BUDGET_ENFORCED,
    /** A data-volume budget (#942) was created, updated or deleted. */
    DATA_BUDGET_CREATED,
    DATA_BUDGET_UPDATED,
    DATA_BUDGET_DELETED,
    ROW_SECURITY_POLICY_CREATED,
    ROW_SECURITY_POLICY_UPDATED,
    ROW_SECURITY_POLICY_DELETED,
    ROW_LIMIT_POLICY_CREATED,
    ROW_LIMIT_POLICY_UPDATED,
    ROW_LIMIT_POLICY_DELETED,
    QUERY_COMMENT_ADDED,
    QUERY_COMMENT_REPLIED,
    QUERY_COMMENT_RESOLVED,
    QUERY_COMMENT_REOPENED,
    DATA_CLASSIFICATION_TAG_ADDED,
    DATA_CLASSIFICATION_TAG_REMOVED,
    COMPLIANCE_REPORT_EXPORTED,
    EXPORT_POLICY_CREATED,
    EXPORT_POLICY_UPDATED,
    EXPORT_POLICY_DELETED,
    RESULT_EXPORTED,
    ORGANIZATION_UPDATED,
    ORGANIZATION_DISABLED,
    ORGANIZATION_ENABLED,
    ANOMALY_ACKNOWLEDGED,
    ANOMALY_DISMISSED,
    DASHBOARD_SUMMARY_EXPORTED,
    ATTESTATION_CAMPAIGN_OPENED,
    ATTESTATION_CAMPAIGN_CLOSED,
    ATTESTATION_CAMPAIGN_CANCELLED,
    ATTESTATION_ITEM_CERTIFIED,
    ATTESTATION_ITEM_REVOKED,
    ATTESTATION_EVIDENCE_EXPORTED,
    OVER_PROVISIONED_ACCESS_EXPORTED,

    /**
     * An admin traced a hypothetical request through the live evaluators, or read who can reach a
     * table (AF-859). Both endpoints are read-only, but they disclose the organization's access
     * topology, so they are audited like the other sensitive reads.
     */
    ACCESS_SIMULATION_RUN,
    /**
     * An admin or auditor read the privileged-access report (#968) — who can reach data with no
     * permission row. Read-only, but it discloses the organization's bypass topology, so it is
     * audited like the other sensitive reads. Metadata: {@code row_count} and the filters applied.
     */
    PRIVILEGED_ACCESS_REPORT_VIEWED,
    API_CONNECTOR_CREATED,
    API_CONNECTOR_UPDATED,
    API_CONNECTOR_DELETED,
    API_CONNECTOR_OAUTH2_TOKEN_REFRESHED,
    API_SCHEMA_UPLOADED,
    API_SCHEMA_DELETED,
    API_PERMISSION_GRANTED,
    API_PERMISSION_UPDATED,
    API_PERMISSION_REVOKED,
    API_PERMISSION_GROUP_GRANTED,
    API_PERMISSION_GROUP_UPDATED,
    API_PERMISSION_GROUP_REVOKED,
    API_REQUEST_SUBMITTED,
    API_REQUEST_APPROVED,
    API_REQUEST_REJECTED,
    API_REQUEST_EXECUTED,
    API_REQUEST_FAILED,
    API_REQUEST_CANCELLED,
    API_REQUEST_BREAK_GLASS_EXECUTED,
    API_BREAK_GLASS_REVIEWED,
    API_CONNECTOR_MASKING_POLICY_CREATED,
    API_CONNECTOR_MASKING_POLICY_UPDATED,
    API_CONNECTOR_MASKING_POLICY_DELETED,
    API_CONNECTOR_CLASSIFICATION_TAG_ADDED,
    API_CONNECTOR_CLASSIFICATION_TAG_REMOVED,
    API_CONNECTOR_VARIABLE_CREATED,
    API_CONNECTOR_VARIABLE_UPDATED,
    API_CONNECTOR_VARIABLE_DELETED,
    API_CONNECTOR_VARIABLES_REORDERED,

    RETENTION_POLICY_CREATED,
    RETENTION_POLICY_UPDATED,
    RETENTION_POLICY_DELETED,
    RETENTION_POLICY_EXECUTED,
    DATA_ERASURE_REQUESTED,
    DATA_ERASURE_APPROVED,
    DATA_ERASURE_REJECTED,
    DATA_ERASURE_CANCELLED,
    DATA_ERASURE_COMPLETED,
    DATA_PSEUDONYMIZED,
    REQUEST_GROUP_SUBMITTED,
    REQUEST_GROUP_APPROVED,
    REQUEST_GROUP_REJECTED,
    REQUEST_GROUP_EXECUTED,
    REQUEST_GROUP_PARTIALLY_EXECUTED,
    REQUEST_GROUP_FAILED,
    REQUEST_GROUP_CANCELLED,

    DISCOVERY_SCAN_COMPLETED,
    DISCOVERY_FINDING_CONFIRMED,
    DISCOVERY_FINDING_DISMISSED,
    DISCOVERY_FINDING_EXPIRED,

    SCIM_CONFIG_UPDATED,
    SCIM_TOKEN_CREATED,
    SCIM_TOKEN_REVOKED,
    SCIM_USER_PROVISIONED,
    SCIM_USER_UPDATED,
    SCIM_USER_DEACTIVATED,
    SCIM_GROUP_SYNCED,
    SCIM_GROUP_DELETED,

    DEPLOYMENT_BREAK_GLASS_EXECUTED,
    DEPLOYMENT_EXECUTED,
    DEPLOYMENT_OUTCOME_REPORTED,
    DEPLOYMENT_ROLLBACK_REVIEWED,
    /** #695: the remaining deployment lifecycle transitions (epic AF-682). */
    DEPLOYMENT_SUBMITTED,
    DEPLOYMENT_APPROVED,
    DEPLOYMENT_REJECTED,
    DEPLOYMENT_TIMED_OUT,
    DEPLOYMENT_CANCELLED,
    /** #695: a deployment break-glass retro-review acknowledged (was generic BREAK_GLASS_REVIEWED). */
    DEPLOYMENT_BREAK_GLASS_REVIEWED,

    /**
     * #880 (epic #870): schema change-set promotion lifecycle. SUBMITTED carries the acting user.
     * APPLIED / PARTIALLY_APPLIED / FAILED are system rows (null actor,
     * {@code trigger=request_group}) projected from the promotion's request group. CANCELLED is
     * written on two paths: by the cancel endpoint with the acting user, and — because a rejected
     * or timed-out group also projects onto the {@code CANCELLED} promotion status — as a system
     * row whose {@code group_status} metadata says whether it was REJECTED, TIMED_OUT or CANCELLED.
     * Do not read a null actor on this action as "unattributed": read {@code group_status}.
     * APPROVED (#882) is a system row too: the group's review was satisfied.
     */
    SCHEMA_CHANGE_PROMOTION_SUBMITTED,
    SCHEMA_CHANGE_PROMOTION_APPROVED,
    SCHEMA_CHANGE_PROMOTION_APPLIED,
    SCHEMA_CHANGE_PROMOTION_PARTIALLY_APPLIED,
    SCHEMA_CHANGE_PROMOTION_FAILED,
    SCHEMA_CHANGE_PROMOTION_CANCELLED,

    /**
     * #881 (epic #870): schema drift. SCAN_COMPLETED is written once per scan of one environment —
     * a null actor with {@code trigger=schedule} for the job, the requesting user with
     * {@code trigger=manual} for "Scan now" — and carries {@code applicable}, {@code partial},
     * {@code findings_count} and, when the scan compared nothing, the {@code reason} code that
     * says why. FINDING_ACKNOWLEDGED and CONFIG_UPDATED always carry the acting user. Drift never
     * writes to a customer database, so there is no remediation action here and never will be.
     */
    SCHEMA_DRIFT_SCAN_COMPLETED,
    SCHEMA_DRIFT_FINDING_ACKNOWLEDGED,
    SCHEMA_DRIFT_CONFIG_UPDATED,

    /**
     * #882 (epic #870): change-set authoring. Every row carries the acting user; UPDATED records the
     * fields the request changed, STATEMENTS_REPLACED the new statement count and checksum, DELETED
     * the name the set had.
     */
    SCHEMA_CHANGE_SET_CREATED,
    SCHEMA_CHANGE_SET_UPDATED,
    SCHEMA_CHANGE_SET_STATEMENTS_REPLACED,
    SCHEMA_CHANGE_SET_DELETED
}
