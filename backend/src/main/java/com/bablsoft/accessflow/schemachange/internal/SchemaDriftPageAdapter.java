 ...
 * {@code Pageable}/{@code Page} inside this module (core's PageAdapter is module-private).
 */
final class SchemaDriftPageAdapter {

    private SchemaDriftPageAdapter() {
    }
    ...
    private static Sort toSpringSort(PageRequest request) {
        ...
        return Sort.by(request.sort().stream().map(SchemaDriftPageAdapter::toSpringOrder).toList());
    }
