 ...
 * test-context cache works, ~90 test classes share a single context that is never idle, so every
 * {@code @Scheduled} job would run continuously against the one shared Postgres and mutate rows
 * other tests are asserting on. No job declares an {@code initialDelay}, so pinning cadences would
 * not suppress the burst at context start. Job tests drive the job method directly instead.
 *
