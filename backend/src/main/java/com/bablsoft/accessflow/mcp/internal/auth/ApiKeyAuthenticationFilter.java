 ...
 *
 * <p>The filter is registered into the main security chain by name (see {@code SecurityConfiguration})
 * and runs before {@code JwtAuthenticationFilter}; if no API key header is present (or it doesn't
 * resolve), the JWT filter still gets a chance to authenticate the request. Failures are silent —
 * the security entry point handles unauthenticated requests.
 */
@Component
