 ...
 * internally), the document-engine analogue of {@code DefaultDatasourceConnectionPoolManager}.
 * Clients are evicted on datasource config-change / deactivation by {@link MongoClientEvictionListener}.
 * SELECT operations on a datasource with a configured read replica are routed
...
class MongoClientManager {
    ...
    private final ProxyMongoProperties properties;
    ...
    private MongoClient create(DatasourceConnectionDescriptor descriptor) {
        var password = descriptor.passwordEncrypted() == null || descriptor.passwordEncrypted().isBlank()
                ? ""
                : encryptionService.decrypt(descriptor.passwordEncrypted());
        var uri = connectionStringFactory.build(descriptor, password, properties.toOptions());
