 ...
 * {@code Pageable}/{@code Page} inside the lifecycle module (core's PageAdapter is module-private).
 */
final class LifecyclePageAdapter {

    private LifecyclePageAdapter() {
    }
    ...
    private static Sort toSpringSort(PageRequest request) {
        ...
        return Sort.by(request.sort().stream().map(LifecyclePageAdapter::toSpringOrder).toList());
    }
