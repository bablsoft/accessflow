...
import { App, Button, DatePicker, Form, Modal, Select, Switch, Table } from 'antd';
...
import type { ApiConnectorPermission, ApiOperation, User } from '@/types/api';
...
export function ApiConnectorPermissionsTab({ connectorId }: { connectorId: string }) {
  ...
  const { message } = App.useApp();
  const queryClient = useQueryClient();
  const [form] = Form.useForm<PermissionCapabilityValues & { user_id: string }>();
  ...
  const grantMutation = useMutation({
    mutationFn: (values: PermissionCapabilityValues & { user_id: string }) =>
      grantApiConnectorPermission(connectorId, {
