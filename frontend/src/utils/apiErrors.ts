...
export function datasourceGrantErrorMessage(err: unknown): string {
  if (axios.isAxiosError(err)) {
    ...
    if (code === 'BYTES_SCANNED_CAP_NOT_SUPPORTED' && body?.detail) return body.detail;
    if (body?.title) return body.title;
    if (body?.detail) return body.detail;
    if (ax.message) return ax.message;
  }
...
export function organizationErrorMessage(err: unknown): string {
  if (axios.isAxiosError(err)) {
    const ax = err as AxiosError<ProblemDetail>;
    const body = ax.response?.data;
    const code = body?.error;
    if (code === 'ORGANIZATION_NOT_FOUND') {
      return i18n.t('errors.organization_not_found');
    }
    if (code === 'QUOTA_EXCEEDED') {
      // The backend already localises the quota detail with the limit interpolated.
      if (body?.detail) return body.detail;
      return i18n.t('errors.quota_exceeded');
    }
    if (body?.title) return body.title;
    if (body?.detail) return body.detail;
    if (ax.message) return ax.message;
