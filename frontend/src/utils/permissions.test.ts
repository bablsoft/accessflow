...
describe('permissions', () => {
  it('exposes the full 48-entry catalog mirror', () => {
    expect(PERMISSIONS).toHaveLength(48);
    expect(new Set(PERMISSIONS).size).toBe(PERMISSIONS.length);
    expect(PERMISSIONS).toContain('EXPORT_POLICY_MANAGE');
    expect(PERMISSIONS).toContain('ROW_LIMIT_POLICY_MANAGE');
    expect(PERMISSIONS).toContain('DATA_BUDGET_MANAGE');
    ...
    expect(PERMISSIONS).toContain('SCHEMA_CHANGE_MANAGE');
  });
